...
int unit_warp(struct block_list *bl,short m,short x,short y,clr_type type)
{
	...
	switch (bl->type) {
		case BL_MOB:
		{
			...
			break;
		case BL_PC:
			if (map->list[bl->m].flag.noteleport || map->getcell(bl->m, bl, bl->x, bl->y, CELL_CHKPVP))
				return 1;
			break;
	}
...
// Do not reset can-act delay. [Skotlex]
	...
	if(sc && sc->count ) { //map-change/warp dispells.
		status_change_end(bl, SC_NARA_03, INVALID_TIMER);
		status_change_end(bl, SC_NARA_03_WAIT, INVALID_TIMER);
