...
int64 battle_attr_fix(struct block_list *src, struct block_list *target, int64 damage,int atk_elem,int def_type, int def_lv)
{
	...
	if (sc && sc->count) {
		...
		if(sc->data[SC_NJ_SUITON] && atk_elem == ELE_WATER)
            		ratio += sc->data[SC_NJ_SUITON]->val4;
...
int battle_calc_skillratio(int attack_type, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int skillratio, int flag){
	...
	switch(attack_type){
		...
		case BF_WEAPON:
			switch( skill_id )
			{
				case BOR_LOTUS_COMBO_4:
				case BOR_LOTUS_COMBO_8:
				case BOR_LOTUS_COMBO_16:
				case BOR_LOTUS_COMBO_32:
				case BOR_LOTUS_COMBO_64:
				case BOR_LOTUS_COMBO_128:
					skillratio += 20 * skill_lv;
					break;
				case SM_BASH:
				case MS_BASH:
				...
				case MS_MAGNUM:
				case BOR_8TRIG:
				case BA_BAIKANOJUTSU:
				case BOR_SUSANOSASUKE:
					skillratio += 20 * skill_lv;
					break;
				case MC_MAMMONITE:
					skillratio += 50 * skill_lv;
				...
				case MO_EXTREMITYFIST:
#ifndef RENEWAL
					{
						...
						//You'd need something like 5K SP to reach this max, so should be fine for most purposes.
						if (ratio > 50000) ratio = 50000; //We leave some room here in case skillratio gets further increased.
...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv) {
	...
	if (t_sd != NULL) {
		...
		if(map->list[t_sd->bl.m].flag.zombie)
			return 100;
	...
	if( sc && sc->count )
	{
		...
		if(sc->data[SC_DODGE_READY] && ( !sc->opt1 || sc->opt1 == OPT1_BURNING ) &&
			...
			if (t_sd && pc_issit(t_sd)) pc->setstand(t_sd); //Stand it to dodge.
				...
				clif->skill_nodamage(bl, bl, BOR_SANIN_HP01, 1, 1);
		...
		&& flag&BF_WEAPON && !(skill->get_nk(skill_id)&NK_NO_CARDFIX_ATK)) {
			...
			if (sce) {
                if((sce=sc->data[SC_NJ_UTSUSEMI])){
                    struct mob_data *summon_md;

		   summon_md = mob->once_spawn_sub(bl, bl->m, bl->x, bl->y, clif->get_bl_name(bl), 1038, "", SZ_SMALL, AI_NONE);
			if (summon_md) {
                        summon_md->master_id = bl->id;
                        ...
                        summon_md->deletetimer = timer->add(timer->gettick() + skill->get_time2(NJ_UTSUSEMI, sce->val1), mob->timer_delete, summon_md->bl.id, 0);
                        mob->spawn( summon_md );
                        sc_start4(NULL, &summon_md->bl, SC_MODECHANGE, 100, 1, 0, 0, MD_AGGRESSIVE|MD_CANATTACK|MD_CANMOVE|MD_ANGRY, 0);
                        pc->setinvincibletimer(t_sd,500);// unlock target lock
...
bool battle_skill_damage_iscaster(struct block_list* src, struct block_list* target, int src_type, int target_type)
{
	int bl_src = 0;
	int bl_target = 0;
	int i = 0 , j = 0;
	int valor_src = 0, valor_target = 0;
	int matriz[7] = { 0,1,2,3,4,5,6 };
	...
	if(bl_src == 0 || bl_target == 0)
		return false;

	for(i = 0; i < ARRAYLENGTH(matriz); i++)
	{
		if (valor_src != src_type) {
			valor_src = valor_src + i;
		}
	...
	for (j = 0; j < ARRAYLENGTH(matriz); j++)
	{
		if (valor_target != target_type) {
			valor_target = valor_target + j;
		}
	...
	if (valor_src < bl_src || valor_target < bl_target)
		return false;

	return true;
}

int battle_skill_damage_skill(struct block_list* src, struct block_list* target, uint16 skill_id, uint16 skill_lv)
{
	unsigned short m = src->m;

	int idx = skill->get_skilldamage_id(skill_id, skill_lv);
	int flag_type = skill->get_skilldamage_flagtype(skill_id, skill_lv);
	int src_type = skill->get_skilldamage_type(skill_id, skill_lv);
	int src_type2 = skill->get_skilldamage_type2(skill_id, skill_lv);

	if (battle->skill_damage_iscaster(src, target, src_type, src_type2) == false) {
		return 0;
	}
	...
	//verifica a flag dos mapas
	switch (flag_type)
	{
	case 1://mapa normal
		if (map_flag_pvp(m) && map_flag_gvg(m) && map_flag_gvg2(m) && map_flag_fvf(m) && map->list[m].flag.battleground)
			return 0;
		break;
	case 2://mapa pvp
		if (!map_flag_pvp(m))
		...
		break;
	case 3://mapa gvg
		if (!map_flag_gvg(m) || !map_flag_gvg2(m))
			return 0;
		break;
	case 4://mapa cvc
		if (!map_flag_fvf(m))
	...
	case 5://battleground
		...
		break;
	case 6://normal + pvp
		if (map_flag_gvg(m) && map_flag_gvg2(m) && map_flag_fvf(m) && map->list[m].flag.battleground)
			return 0;
		break;
	case 7://normal + gvg
		if (map_flag_pvp(m) && map_flag_fvf(m) && map->list[m].flag.battleground)
			return 0;
		break;
	case 8://normal + cvc
		if (map_flag_pvp(m) && map_flag_gvg(m) && map_flag_gvg2(m) && map->list[m].flag.battleground)
			return 0;
		break;
	case 9://normal + battleground
		if (map_flag_pvp(m) && map_flag_gvg(m) && map_flag_gvg2(m) && map_flag_fvf(m))
			return 0;
		break;
	case 10://pvp + gvg + cvc
		if (!map_flag_pvp(m) && !map_flag_gvg(m) && !map_flag_gvg2(m) && !map_flag_fvf(m))
	...
	case 11://todos
		break;
	default:
		ShowWarning("skill_damage_db: flag %d inválido! use um valor de 0 a 11\n", flag_type);
		break;
	...
	switch (target->type) {
	case BL_PC:
		return skill->get_skilldamage_danoplayer(skill_id, skill_lv);
	case BL_MOB:
		if (is_boss(target)) {
			return skill->get_skilldamage_danomvp(skill_id, skill_lv);
		}
		else {
			return skill->get_skilldamage_danomonstro(skill_id, skill_lv);
		}
	default:
		return skill->get_skilldamage_outros(skill_id, skill_lv);
	}

	return 0;
}

int battle_skill_damage_map(struct block_list* src, struct block_list* target, uint16 skill_id, uint16 skill_lv)
{
	int rate = 0, flag_type = 0, src_type = 0, src_type2 = 0;

	int16 m = map->mapname2mapid(mapindex_id2name(map->list[src->m].index));

	for (int i = 0; i < MAX_MAP_SKILL_MODIFIER; i++) {
		if (map->list[m].skill_damage[i].skill_id == skill_id) {
			flag_type = map->list[m].skill_damage[i].flag_type;
			src_type = map->list[m].skill_damage[i].caster;
			src_type2 = map->list[m].skill_damage[i].caster2;
			break;
		}
	}
	...
	case 11://todos
		break;
	default:
		ShowWarning("battle_skill_damage_map: flag %d inválido! use um valor de 0 a 11\n", flag_type);
		break;
	...
	switch (target->type) {
	case BL_PC:
		rate = map->list[m].adjust.damage.pc;
		break;
	case BL_MOB:
		if (is_boss(target))
			rate = map->list[m].adjust.damage.boss;
		else
			rate = map->list[m].adjust.damage.mob;
		break;
	default:
		rate = map->list[m].adjust.damage.other;
		break;
	}
	...
	ARR_FIND(0, MAX_MAP_SKILL_MODIFIER, i, map->list[m].skill_damage[i].skill_id == skill_id);
	if (i < MAX_MAP_SKILL_MODIFIER) {
		switch (target->type) {
		case BL_PC:
			rate += map->list[m].skill_damage[i].pc;
			break;
		case BL_MOB:
			if (is_boss(target))
				rate += map->list[m].skill_damage[i].boss;
			else
				rate += map->list[m].skill_damage[i].mob;
			break;
		default:
			rate += map->list[m].skill_damage[i].other;
			break;
		}
	}
...
int battle_skill_damage(struct block_list* src, struct block_list* target, uint16 skill_id, uint16 skill_lv)
{
	if (!target)
		return 0;

	return battle->skill_damage_skill(src, target, skill_id, skill_lv) + battle->skill_damage_map(src, target, skill_id, skill_lv);
}
#endif
...
#ifdef ADJUST_SKILL_DAMAGE
	if ((skill_damage = battle->skill_damage(src, target, skill_id, skill_lv)) != 0)
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int mflag) {
	...
	case CR_ACIDDEMONSTRATION:
		...
		if(tstatus->vit+sstatus->int_) //crash fix
			md.damage = (int)(14*tstatus->vit*sstatus->int_*sstatus->int_ / (10*(tstatus->vit+sstatus->int_)));
		else
			md.damage = 0;
		if (tsd) md.damage>>=1;
#endif
...
struct Damage battle_calc_weapon_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int wflag)
{
	...
	if ((!skill_id || skill_id == PA_SACRIFICE || skill_id == BOR_SANIN_GP01) && tstatus->flee2 && rnd()%1000 < tstatus->flee2) {
...
#define ATK_ADDRATE3(damage, damage2, a) do { int64 rate_ = (a); (damage) += (damage) * rate_ / 100; if(flag.lh) (damage2) += (damage2) *rate_ / 100; } while(0);
//Adds an absolute value to damage. 100 = +100 damage
...
#define GET_NORMAL_ATTACK2( f , s ) ( wd.damage2 = battle->calc_base_damage(src, target, s, skill_lv, nk, n_ele, s_ele, s_ele_, EQI_HAND_L, (f), wd.flag) )
		...
		switch (skill_id) {
			...
			case HFLI_SBR44: //[orn]
				...
				break;
			case NJ_SYURIKEN:
                			wd.damage = 1*sstatus->batk / 2;
                			ATK_ADD(10*sstatus->agi + 10*sstatus->dex);
                			break;
			case NJ_KUNAI:
                			wd.damage = 1*sstatus->batk / 2;
                			ATK_ADD(10*sstatus->agi + 10*sstatus->dex);
			...
			case GC_DARKILLUSION:
                			...
                			break;
			case TK_COUNTER:
                			wd.damage = 8*sstatus->batk;
                			ATK_ADD(35*sstatus->dex + 25*sstatus->agi);
                			break;
			case SR_DRAGONCOMBO:
                			wd.damage = 12*sstatus->batk;
                			ATK_ADD(20*sstatus->agi + 10*sstatus->dex);
                			break;
			case TK_JUMPKICK:
                			wd.damage = 2*sstatus->batk;
                			ATK_ADD(20*sstatus->dex + 20*sstatus->agi);
                			break;
			case LG_EARTHDRIVE:
                			wd.damage = 13*sstatus->batk;
                			break;
			case TK_STORMKICK:
                			wd.damage = 8*sstatus->batk;
                			ATK_ADD(35*sstatus->dex + 25*sstatus->agi);
			...
			case SR_FALLENEMPIRE:
                			wd.damage = 8*sstatus->batk;
					ATK_ADD(15*sstatus->agi + 10*sstatus->dex);
                			break;
			case SR_WINDMILL:
                			wd.damage = 2*sstatus->batk;
                			ATK_ADD(20*sstatus->dex + 20*sstatus->agi);
			...
			case CH_PALMSTRIKE:
                			wd.damage = 8*sstatus->batk;
                			ATK_ADD(20*sstatus->dex + 15*sstatus->agi);
                			break;
			case TK_TURNKICK:
                			wd.damage = 8*sstatus->batk;
                			ATK_ADD(35*sstatus->dex + 25*sstatus->agi);
			...
			case SR_TIGERCANNON:
				if( sd ) {
                			wd.damage = 6*sstatus->batk;
                			ATK_ADD(30*sstatus->agi + 15*sstatus->dex + 10*sstatus->luk);
				}
					if( sc && sc->data[SC_COMBOATTACK]
					&& sc->data[SC_COMBOATTACK]->val1 == SR_FALLENEMPIRE )
                			wd.damage = 12*sstatus->batk;
                			ATK_ADD(60*sstatus->agi + 30*sstatus->dex + 20*sstatus->luk);
                			break;
			case SR_SKYNETBLOW:
                			wd.damage = 2*sstatus->batk;
                			ATK_ADD(20*sstatus->dex + 20*sstatus->agi);
                			break;
			case SR_EARTHSHAKER:
                			wd.damage = 15*sstatus->batk;
                			ATK_ADD(10*sstatus->dex + 10*sstatus->int_ + 10*sstatus->luk);
                			break;
			case MO_INVESTIGATE://MEDICINA
                			wd.damage = 7*sstatus->batk;
                			ATK_ADD(15*sstatus->agi + 15*sstatus->dex + 10*sstatus->luk);
                			break;
			default:
			{
				i = (flag.cri
...
#ifdef ADJUST_SKILL_DAMAGE
	if ((skill_damage = battle->skill_damage(src, target, skill_id, skill_lv)) != 0)
		ATK_ADDRATE3(wd.damage, wd.damage2, skill_damage);
#endif
...
enum damage_lv battle_weapon_attack(struct block_list* src, struct block_list* target, int64 tick, int flag) {
	...
	if (tsc && tsc->data[SC_NARA_03_WAIT] && !is_boss(src) && (src->type == BL_PC || tsd == NULL || distance_bl(src, target) <= (tsd->status.weapon == W_FIST ? 1 : 2)))
	{
		uint16 skill_lv = tsc->data[SC_NARA_03_WAIT]->val1;
		int duration = skill->get_time2(BOR_NARA_03, skill_lv);
		status_change_end(target, SC_NARA_03_WAIT, INVALID_TIMER);
		if (sc_start4(target, src, SC_NARA_03, 100, sd ? pc->checkskill(sd, BOR_NARA_03) : 5, 0, 0, target->id, duration)) {
			//Target locked.
			clif->damage(src, target, sstatus->amotion, 1, 0, 1, BDT_NORMAL, 0); //Display MISS.
			clif->bladestop(target, src->id, 1);
			sc_start4(target, target, SC_NARA_03, 100, skill_lv, 0, 0, src->id, duration);
			return ATK_BLOCK;
		}
	}
	...
	if (sd && (skillv = pc->checkskill(sd, BOR_SANIN_GP01)) > 0) {
		int triple_rate = battle_config.gama_01_hit_chance; //Base Rate
		if (rnd() % 100 < triple_rate) {
			if (skill->attack(BF_WEAPON, src, src, target, BOR_SANIN_GP01, skillv, tick, 0))
				return ATK_DEF;
			return ATK_MISS;
		}
	}
	if (sd && (skillv = pc->checkskill(sd, BOR_SANIN_GP02)) > 0) {
		int triple_rate = battle_config.gama_02_hit_chance; //Base Rate
		if (rnd() % 100 < triple_rate) {
			if (skill->attack(BF_WEAPON, src, src, target, BOR_SANIN_GP02, skillv, tick, 0)) {
				sc_start(src, target, SC_BLOODING, battle_config.gp02_chance, skillv, battle_config.gp02_time);
				return ATK_DEF;
			}
			return ATK_MISS;
		}
	}
	if (sd && (skillv = pc->checkskill(sd, BOR_SANIN_HP01)) > 0) {
		int triple_rate = battle_config.hebi_01_hit_chance; //Base Rate
		if (rnd() % 100 < triple_rate) {
			if (!(sc->data[SC_SANIN_H01]) && clif->skill_nodamage(src, src, BOR_SANIN_HP01, skillv, sc_start(src, src, SC_DODGE_READY, battle_config.hp01_chance, skillv, 10000)))
				if (clif->skill_nodamage(src, src, BOR_SANIN_HP01, skillv, sc_start(src, src, SC_SANIN_H01, 1000, 1, 10000)))
					return ATK_DEF;
				return ATK_MISS;
		}
	}
	if (sd && (skillv = pc->checkskill(sd, BOR_SANIN_HP02)) > 0) {
		int triple_rate = battle_config.hebi_02_hit_chance; //Base Rate
		if (rnd() % 100 < triple_rate) {
			if (skill->attack(BF_WEAPON, src, src, target, BOR_SANIN_HP02, skillv, tick, 0))
				sc_start(src, target, SC_BLOODING, battle_config.hp02_chance, skillv, battle_config.hp02_time);
				sc_start(src, src, SC_INCHITRATE, battle_config.hp02_agi_rate, skillv, battle_config.hp02_agi_time);
				return ATK_DEF;
			return ATK_MISS;
		}
	}
	...
	if (sc) {
		...
		if(sc->data[SC_GENTLETOUCH_CHANGE])
            		sc_start2(src,target,SC_CRITICALWOUND,50,1,sc->data[SC_GENTLETOUCH_CHANGE]->val3,skill->get_time2(status->sc2skill(SC_GENTLETOUCH_CHANGE),sc->data[SC_GENTLETOUCH_CHANGE]->val1));
        	if(sc->data[SC_LKCONCENTRATION])
            		sc_start2(src,target,SC_BLOODING,50,1,sc->data[SC_LKCONCENTRATION]->val3,skill->get_time2(status->sc2skill(SC_LKCONCENTRATION),sc->data[SC_LKCONCENTRATION]->val1));
	...
	if (tsc) {
		...
		if (tsc->data[SC_GEN01]) {
			uint16 skill_id = 1034;
			uint16 skill_lv = 1;
			sc_start(src, src, SC_BLIND, (int)battle_config.gen_01_chance, skill_lv, skill->get_time(skill_id, skill_lv));
			skill->castend_nodamage_id(target, target, skill_id, skill_lv, tick, flag);
			status_change_end(target, SC_GEN01, INVALID_TIMER);
		}
		else if (tsc->data[SC_GEN02]) {
			uint16 skill_id = 1035;
			uint16 skill_lv = 1;
			sc_start(src, src, SC_STUN, (int)battle_config.gen_02_chance, skill_lv, skill->get_time(skill_id, skill_lv));
			skill->castend_nodamage_id(target, target, skill_id, skill_lv, tick, flag);
			status_change_end(target, SC_GEN02, INVALID_TIMER);
		}
	}
	map->freeblock_unlock();
	return wd.dmg_lv;
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	//Bloqueia ataque de player a monstros de sua facção
	if ((src->type == BL_PC && (BL_UCCAST(BL_MOB, target)->clan_id > 0) && ((BL_UCCAST(BL_MOB, target)->clan_id) == (BL_CAST(BL_PC, src)->status.clan_id)) && target->type == BL_MOB) || (src->type == BL_PC && ((BL_UCCAST(BL_MOB, target)->clan_id) == 5) && (BL_CAST(BL_PC, src)->status.clan_id < 1) && target->type == BL_MOB))
		return 0;

	//Bloqueia ataque de monstros a players de sua facção
	if (src->type == BL_MOB && (BL_CAST(BL_MOB, src)->clan_id > 0) && ((BL_CAST(BL_MOB, src)->clan_id) == (BL_UCCAST(BL_PC, target)->status.clan_id)) && target->type == BL_PC)
		return 0;

	//Bloqueia ataque de monstros clone do byakugan
	if ((BL_UCCAST(BL_MOB, src)->ud.state.byak_control > 0))
		return 0;

	if (!map->list[m].flag.pvp)
		if (src->type == BL_MOB && (BL_CAST(BL_MOB, src)->FakePlayer) && target->type == BL_HOM)
			return 0;
	...
	switch( target->type ) { // Checks on actual target
		...
		{
			const struct mob_data *md = BL_UCCAST(BL_MOB, target);
			const struct unit_data* ud = unit->bl2ud(target);

			if (md->Hateffectmob || md->state.blockimmune || ud && ud->immune_attack || src->type == BL_MOB && md->class_ == 2337)
				return -1;
	...
	switch( t_bl->type ) { //Checks on target master
		case BL_PC:
		{
			...
			if( sd->disguise != -1 && map->list[sd->bl.m].flag.zombie && flag&BCT_ENEMY  )
				return 0;
...
static const struct battle_data {
	...
	{ "pet_disable_in_pvp",                 &battle_config.pet_no_pvp,                      0,      0,      1,              },
	{ "pet_disable_in_fvf",                 &battle_config.pet_no_fvf,                      0,      0,      1,              },
...
//	{ "rare_drop_announce",                 &battle_config.rare_drop_announce,              0,      0,      10000,          },
	...
	{ "feature.rodex",						&battle_config.feature_rodex,                   1,      0,      1,				},
	{ "feature.rodex_use_accountmail",		&battle_config.feature_rodex_use_accountmail,   0,      0,      1,				},
	{ "feature.enable_homun_autofeed",      &battle_config.feature_enable_homun_autofeed,   1,      0,      1,				},
	{ "feature.enable_pet_autofeed",        &battle_config.feature_enable_pet_autofeed,     1,      0,      1,              },
	{ "storage_use_item",                   &battle_config.storage_use_item,                0,      0,      1,              },
	{ "feature.enable_attendance_system",   &battle_config.feature_enable_attendance_system,1,      0,      1,              },
	{ "feature.feature_attendance_endtime", &battle_config.feature_attendance_endtime,      1,      0,      99999999,       },
	...
	{ "show_monster_hp_bar",                &battle_config.show_monster_hp_bar,             1,      0,      1|2|3|4,        },
	{ "all_hp_monster_map",                 &battle_config.all_hp_monster_map,              1,      0,      1,              },
	...
	{ "storage_auto_new_entry",				&battle_config.storage_auto_new_entry, 			0,      0,      1,				},
	/*---------------------------------------------------
	...
	**/
	...
	{ "item_universe",						&battle_config.item_universe,					0,		0,		MAX_ITEMDB,		},
	// desativar teleporte nos mapas com mvp
	{ "warp_block_coordinates_map_mvp",		&battle_config.warp_block_coordinates_map_mvp, 	0,      0,      1,				},
	// desativar imunidade da GTB ao dispell
	{ "desactive_gtb_imunity_dispell",		&battle_config.desactive_gtb_imunity_dispell, 	0,      0,      2,				},
	//marcação de minimapa no mobsearch
	{ "mobsearch_minimap_point",			&battle_config.mobsearch_minimap_point, 	0,      0,      2,				},
	//emblemas de clã
	{ "emblem_fac_sys",						&battle_config.emblem_fac_sys, 	0,      0,      2, },
	// anti macro
	{ "action_keyboard_limit",              &battle_config.action_keyboard_limit,           0,   	0,      10000, },
	{ "action_keyboard_limit_item",         &battle_config.action_keyboard_limit_item,      0,   	0,      10000, },
	{ "action_mouse_limit",                 &battle_config.action_mouse_limit,              0,   	0,      10000, },
	{ "action_dual_limit",                  &battle_config.action_dual_limit,               0,   	0,      10000, },
	{ "hansip_protection",				    &battle_config.hansip_protection,				1,      0,      1, },
	{ "hansip_min_delay",                   &battle_config.hansip_min_delay,               20,		0,      INT_MAX, },
	{ "hansip_spam_count",                  &battle_config.hansip_spam_count,			   10,		1,      100, },
	{ "hansip_spam_punish",                 &battle_config.hansip_spam_punish,				1,		0,      1, },
	{ "macropunit",					        &battle_config.macropunit,				0,		0,      3, },
	//mapa pvp liberado pet
	{ "active_mappet",		                &battle_config.active_mappet, 	                  0,        0,                        1,         },
	//Sistema de refino de asas
	{ "enable_refinery_ui",					&battle_config.enable_refinery_ui,              1,      0,      1, },
	{ "replace_refine_npcs",				&battle_config.replace_refine_npcs,             1,      0,      1, },
	{ "change_cash_to_refine",				&battle_config.change_cash_to_refine,           1,      0,      2, },
	{ "block_weapons_refine",				&battle_config.block_weapons_refine,            1,      0,      1, },
	{ "id_item_refine_apartir",				&battle_config.id_item_refine_apartir,          0,      0,      MAX_ITEMDB, },
	{ "active_meteor_animation",			&battle_config.active_meteor_animation,         1,      0,      1, },
	{ "active_wall_protect_land",			&battle_config.active_wall_protect_land,        1,      0,      1, },
	{ "active_pneuma_protect_land",			&battle_config.active_pneuma_protect_land,      1,      0,      1, },
	//Rebuild congelar/petrificar mdef e luk
	{ "ero_max_status",                    &battle_config.ero_max_status,                 0,      0,      INT_MAX, },
	{ "ero_cong_formula",                  &battle_config.ero_cong_formula,               0,      0,      INT_MAX, },
	{ "ero_petri_formula",                 &battle_config.ero_petri_formula,              0,      0,      INT_MAX, },
	{ "ero_cong_formula_luk",              &battle_config.ero_cong_formula_luk,           0,      0,      INT_MAX, },
	{ "ero_cong_formula_mdef",             &battle_config.ero_cong_formula_mdef,          0,      0,      INT_MAX, },
	{ "ero_petri_formula_luk",             &battle_config.ero_petri_formula_luk,          0,      0,      INT_MAX, },
	{ "ero_petri_formula_mdef",            &battle_config.ero_petri_formula_mdef,         0,      0,      INT_MAX, },
	{ "ero_cong_formula_fix",              &battle_config.ero_cong_formula_fix,           0,      0,      INT_MAX, },
	{ "ero_petri_formula_fix",             &battle_config.ero_petri_formula_fix,          0,      0,      INT_MAX, },
	{ "ero_cong_des_t2",                   &battle_config.ero_cong_des_t2,                0,      0,      INT_MAX, },
	{ "ero_petri_des_t2",                  &battle_config.ero_petri_des_t2,               0,      0,      INT_MAX, },
	{ "ero_max_luk_cong",                  &battle_config.ero_max_luk_cong,               0,      0,      INT_MAX, },
	{ "ero_max_luk_petri",                 &battle_config.ero_max_luk_petri,              0,      0,      INT_MAX, },
	{ "ero_max_divi",                      &battle_config.ero_max_divi,                   0,      0,      INT_MAX, },
	{ "ero_mdef_divi",                     &battle_config.ero_mdef_divi,                  0,      0,      INT_MAX, },
	{ "ero_mdef2_divi",                    &battle_config.ero_mdef2_divi,                 0,      0,      INT_MAX, },
	{ "ero_fs_mdef",                       &battle_config.ero_fs_mdef,                    0,      0,      INT_MAX, },
	...
	//Espadachim
	{ "d_SM_BASH",							&battle_config.d_SM_BASH,               100,		0,      INT_MAX, },
	{ "d_SM_MAGNUM",						&battle_config.d_SM_MAGNUM,               100,		0,      INT_MAX, },
	//Mago
	{ "d_MG_NAPALMBEAT",                   &battle_config.d_MG_NAPALMBEAT,               100,		0,      INT_MAX, },
	{ "d_MG_SOULSTRIKE",                   &battle_config.d_MG_SOULSTRIKE,               100,		0,      INT_MAX, },
	{ "d_MG_FROSTDIVER",                   &battle_config.d_MG_FROSTDIVER,               100,		0,      INT_MAX, },
	{ "d_MG_STONECURSE",                   &battle_config.d_MG_STONECURSE,               100,		0,      INT_MAX, },
	{ "d_MG_FIREBALL",                   &battle_config.d_MG_FIREBALL,               100,		0,      INT_MAX, },
	{ "d_MG_FIREWALL",                   &battle_config.d_MG_FIREWALL,               100,		0,      INT_MAX, },
	{ "d_MG_THUNDERSTORM",                   &battle_config.d_MG_THUNDERSTORM,               100,		0,      INT_MAX, },
	{ "d_MG_COLDBOLT",                   &battle_config.d_MG_COLDBOLT,               100,		0,      INT_MAX, },
	{ "d_MG_FIREBOLT",                   &battle_config.d_MG_FIREBOLT,               100,		0,      INT_MAX, },
	{ "d_MG_LIGHTNINGBOLT",                   &battle_config.d_MG_LIGHTNINGBOLT,               100,		0,      INT_MAX, },
	//noviço
	{ "d_AL_DECAGI",                   &battle_config.d_AL_DECAGI,               100,		0,      INT_MAX, },
	{ "d_AL_CRUCIS",                   &battle_config.d_AL_CRUCIS,               100,		0,      INT_MAX, },
	{ "d_AL_HOLYLIGHT",                   &battle_config.d_AL_HOLYLIGHT,               100,		0,      INT_MAX, },
	//Mercador
	{ "d_MC_MAMMONITE",                   &battle_config.d_MC_MAMMONITE,               100,		0,      INT_MAX, },
	{ "d_MC_CARTREVOLUTION",                   &battle_config.d_MC_CARTREVOLUTION,               100,		0,      INT_MAX, },
	//Arqueiro
	{ "d_AC_DOUBLE",                   &battle_config.d_AC_DOUBLE,               100,		0,      INT_MAX, },
	{ "d_AC_SHOWER",                   &battle_config.d_AC_SHOWER,               100,		0,      INT_MAX, },
	{ "d_AC_CHARGEARROW",                   &battle_config.d_AC_CHARGEARROW,               100,		0,      INT_MAX, },
	//Gatuno
	{ "d_TF_POISON",                   &battle_config.d_TF_POISON,               100,		0,      INT_MAX, },
	{ "d_TF_BACKSLIDING",                   &battle_config.d_TF_BACKSLIDING,               100,		0,      INT_MAX, },
	//Cavaleiro
	{ "d_KN_PIERCE",                   &battle_config.d_KN_PIERCE,               100,		0,      INT_MAX, },
	{ "d_KN_BRANDISHSPEAR",                   &battle_config.d_KN_BRANDISHSPEAR,               100,		0,      INT_MAX, },
	{ "d_KN_SPEARSTAB",                   &battle_config.d_KN_SPEARSTAB,               100,		0,      INT_MAX, },
	{ "d_KN_SPEARBOOMERANG",                   &battle_config.d_KN_SPEARBOOMERANG,               100,		0,      INT_MAX, },
	{ "d_KN_CHARGEATK",                   &battle_config.d_KN_CHARGEATK,               100,		0,      INT_MAX, },
	{ "d_KN_BOWLINGBASH",                   &battle_config.d_KN_BOWLINGBASH,               100,		0,      INT_MAX, },
	//Sacerdote
	{ "d_PR_LEXDIVINA",                   &battle_config.d_PR_LEXDIVINA,               100,		0,      INT_MAX, },
	{ "d_PR_TURNUNDEAD",                   &battle_config.d_PR_TURNUNDEAD,               100,		0,      INT_MAX, },
	{ "d_PR_LEXAETERNA",                   &battle_config.d_PR_LEXAETERNA,               100,		0,      INT_MAX, },
	{ "d_PR_MAGNUS",                   &battle_config.d_PR_MAGNUS,               100,		0,      INT_MAX, },
	{ "d_PR_REDEMPTIO",                   &battle_config.d_PR_REDEMPTIO,               100,		0,      INT_MAX, },
	//bruxo
	{ "d_WZ_FIREPILLAR",                   &battle_config.d_WZ_FIREPILLAR,               100,		0,      INT_MAX, },
	{ "d_WZ_SIGHTRASHER",                   &battle_config.d_WZ_SIGHTRASHER,               100,		0,      INT_MAX, },
	{ "d_WZ_FIREIVY",                   &battle_config.d_WZ_FIREIVY,               100,		0,      INT_MAX, },
	{ "d_WZ_METEOR",                   &battle_config.d_WZ_METEOR,               100,		0,      INT_MAX, },
	{ "d_WZ_VERMILION",                   &battle_config.d_WZ_VERMILION,               100,		0,      INT_MAX, },
	{ "d_WZ_WATERBALL",                   &battle_config.d_WZ_WATERBALL,               100,		0,      INT_MAX, },
	{ "d_WZ_ICEWALL",                   &battle_config.d_WZ_ICEWALL,               100,		0,      INT_MAX, },
	{ "d_WZ_FROSTNOVA",                   &battle_config.d_WZ_FROSTNOVA,               100,		0,      INT_MAX, },
	{ "d_WZ_STORMGUST",                   &battle_config.d_WZ_STORMGUST,               100,		0,      INT_MAX, },
	{ "d_WZ_EARTHSPIKE",                   &battle_config.d_WZ_EARTHSPIKE,               100,		0,      INT_MAX, },
	{ "d_WZ_HEAVENDRIVE",                   &battle_config.d_WZ_HEAVENDRIVE,               100,		0,      INT_MAX, },
	{ "d_WZ_QUAGMIRE",                   &battle_config.d_WZ_QUAGMIRE,               100,		0,      INT_MAX, },
	{ "d_WZ_ESTIMATION",                   &battle_config.d_WZ_ESTIMATION,               100,		0,      INT_MAX, },
	{ "d_WZ_SIGHTBLASTER",                   &battle_config.d_WZ_SIGHTBLASTER,               100,		0,      INT_MAX, },
	{ "d_WZ_JUPITEL",                   &battle_config.d_WZ_JUPITEL,               100,		0,      INT_MAX, },
	//Ferreiro
	{ "d_BS_HAMMERFALL",                   &battle_config.d_BS_HAMMERFALL,               100,		0,      INT_MAX, },
	//Caçador
	{ "d_HT_BLITZBEAT",                   &battle_config.d_HT_BLITZBEAT,               100,		0,      INT_MAX, },
	{ "d_HT_POWER",                   &battle_config.d_HT_POWER,               100,		0,      INT_MAX, },
	{ "d_HT_PHANTASMIC",                   &battle_config.d_HT_PHANTASMIC,               100,		0,      INT_MAX, },
	//Assasino
	{ "d_AS_GRIMTOOTH",                   &battle_config.d_AS_GRIMTOOTH,               100,		0,      INT_MAX, },
	{ "d_AS_VENOMKNIFE",                   &battle_config.d_AS_VENOMKNIFE,               100,		0,      INT_MAX, },
	{ "d_AS_SONICBLOW",                   &battle_config.d_AS_SONICBLOW,               100,		0,      INT_MAX, },
	//Arruaceiro
	{ "d_RG_BACKSTAP",                   &battle_config.d_RG_BACKSTAP,               100,		0,      INT_MAX, },
	{ "d_RG_RAID",                   &battle_config.d_RG_RAID,               100,		0,      INT_MAX, },
	{ "d_RG_GRAFFITI",                   &battle_config.d_RG_GRAFFITI,               100,		0,      INT_MAX, },
	{ "d_RG_FLAGGRAFFITI",                   &battle_config.d_RG_FLAGGRAFFITI,               100,		0,      INT_MAX, },
	{ "d_RG_COMPULSION",                   &battle_config.d_RG_COMPULSION,               100,		0,      INT_MAX, },
	{ "d_RG_PLAGIARISM",                   &battle_config.d_RG_PLAGIARISM,               100,		0,      INT_MAX, },
	{ "d_RG_CLOSECONFINE",                   &battle_config.d_RG_CLOSECONFINE,               100,		0,      INT_MAX, },
	//Alquimista
	{ "d_AM_DEMONSTRATION",                   &battle_config.d_AM_DEMONSTRATION,               100,		0,      INT_MAX, },
	{ "d_AM_ACIDTERROR",                   &battle_config.d_AM_ACIDTERROR,               100,		0,      INT_MAX, },
	{ "d_AM_POTIONPITCHER",                   &battle_config.d_AM_POTIONPITCHER,               100,		0,      INT_MAX, },
	{ "d_AM_CANNIBALIZE",                   &battle_config.d_AM_CANNIBALIZE,               100,		0,      INT_MAX, },
	{ "d_AM_SPHEREMINE",                   &battle_config.d_AM_SPHEREMINE,               100,		0,      INT_MAX, },
	{ "d_AM_FLAMECONTROL",                   &battle_config.d_AM_FLAMECONTROL,               100,		0,      INT_MAX, },
	{ "d_AM_DRILLMASTER",                   &battle_config.d_AM_DRILLMASTER,               100,		0,      INT_MAX, },
	//Templário
	{ "d_CR_SHIELDBOOMERANG",                   &battle_config.d_CR_SHIELDBOOMERANG,               100,		0,      INT_MAX, },
	{ "d_CR_HOLYCROSS",                   &battle_config.d_CR_HOLYCROSS,               100,		0,      INT_MAX, },
	{ "d_CR_GRANDCROSS",                   &battle_config.d_CR_GRANDCROSS,               100,		0,      INT_MAX, },
	//Monge
	{ "d_MO_CALLSPIRITS",                   &battle_config.d_MO_CALLSPIRITS,               100,		0,      INT_MAX, },
	{ "d_MO_ABSORBSPIRITS",                   &battle_config.d_MO_ABSORBSPIRITS,               100,		0,      INT_MAX, },
	{ "d_MO_INVESTIGATE",                   &battle_config.d_MO_INVESTIGATE,               100,		0,      INT_MAX, },
	{ "d_MO_EXPLOSIONSPIRITS",                   &battle_config.d_MO_EXPLOSIONSPIRITS,               100,		0,      INT_MAX, },
	{ "d_MO_EXTREMITYFIST",                   &battle_config.d_MO_EXTREMITYFIST,               100,		0,      INT_MAX, },
	{ "d_MO_CHAINCOMBO",                   &battle_config.d_MO_CHAINCOMBO,               100,		0,      INT_MAX, },
	{ "d_MO_COMBOFINISH",                   &battle_config.d_MO_COMBOFINISH,               100,		0,      INT_MAX, },
	{ "d_MO_KITRANSLATION",                   &battle_config.d_MO_KITRANSLATION,               100,		0,      INT_MAX, },
	{ "d_MO_BALKYOUNG",                   &battle_config.d_MO_BALKYOUNG,               100,		0,      INT_MAX, },
	{ "d_MO_FINGEROFFENSIVE",                   &battle_config.d_MO_FINGEROFFENSIVE,               100,		0,      INT_MAX, },
	{ "d_MO_BODYRELOCATION",                   &battle_config.d_MO_BODYRELOCATION,               100,		0,      INT_MAX, },
	//Sábio
	{ "d_SA_CASTCANCEL",                   &battle_config.d_SA_CASTCANCEL,               100,		0,      INT_MAX, },
	{ "d_SA_SPELLBREAKER",                   &battle_config.d_SA_SPELLBREAKER,               100,		0,      INT_MAX, },
	{ "d_SA_MONOCELL",                   &battle_config.d_SA_MONOCELL,               100,		0,      INT_MAX, },
	{ "d_SA_CLASSCHANGE",                   &battle_config.d_SA_CLASSCHANGE,               100,		0,      INT_MAX, },
	{ "d_SA_SUMMONMONSTER",                   &battle_config.d_SA_SUMMONMONSTER,               100,		0,      INT_MAX, },
	{ "d_SA_REVERSEORCISH",                   &battle_config.d_SA_REVERSEORCISH,               100,		0,      INT_MAX, },
	{ "d_SA_DEATH",                   &battle_config.d_SA_DEATH,               100,		0,      INT_MAX, },
	{ "d_SA_FORTUNE",                   &battle_config.d_SA_FORTUNE,               100,		0,      INT_MAX, },
	{ "d_SA_TAMINGMONSTER",                   &battle_config.d_SA_TAMINGMONSTER,               100,		0,      INT_MAX, },
	{ "d_SA_QUESTION",                   &battle_config.d_SA_QUESTION,               100,		0,      INT_MAX, },
	{ "d_SA_GRAVITY",                   &battle_config.d_SA_GRAVITY,               100,		0,      INT_MAX, },
	{ "d_SA_LEVELUP",                   &battle_config.d_SA_LEVELUP,               100,		0,      INT_MAX, },
	{ "d_SA_INSTANTDEATH",                   &battle_config.d_SA_INSTANTDEATH,               100,		0,      INT_MAX, },
	{ "d_SA_FULLRECOVERY",                   &battle_config.d_SA_FULLRECOVERY,               100,		0,      INT_MAX, },
	{ "d_SA_COMA",                   &battle_config.d_SA_COMA,               100,		0,      INT_MAX, },
	{ "d_SA_DISPELL",                   &battle_config.d_SA_DISPELL,               100,		0,      INT_MAX, },
	//habilidades comum a bardo e dançarina
	{ "d_BD_ADAPTATION",                   &battle_config.d_BD_ADAPTATION,               100,		0,      INT_MAX, },
	{ "d_BD_ENCORE",                   &battle_config.d_BD_ENCORE,               100,		0,      INT_MAX, },
	{ "d_BD_LULLABY",                   &battle_config.d_BD_LULLABY,               100,		0,      INT_MAX, },
	{ "d_BD_RICHMANKIM",                   &battle_config.d_BD_RICHMANKIM,               100,		0,      INT_MAX, },
	//bardo
	{ "d_BA_MUSICALSTRIKE",                   &battle_config.d_BA_MUSICALSTRIKE,               100,		0,      INT_MAX, },
	{ "d_BA_DISSONANCE",                   &battle_config.d_BA_DISSONANCE,               100,		0,      INT_MAX, },
	{ "d_BA_FROSTJOKER",                   &battle_config.d_BA_FROSTJOKER,               100,		0,      INT_MAX, },
	{ "d_BA_WHISTLE",                   &battle_config.d_BA_WHISTLE,               100,		0,      INT_MAX, },
	{ "d_BA_ASSASSINCROSS",                  &battle_config.d_BA_ASSASSINCROSS,               100,		0,      INT_MAX, },
	{ "d_BA_POEMBRAGI",                   &battle_config.d_BA_POEMBRAGI,               100,		0,      INT_MAX, },
	{ "d_BA_APPLEIDUN",                   &battle_config.d_BA_APPLEIDUN,               100,		0,      INT_MAX, },
	{ "d_BA_PANGVOICE",                   &battle_config.d_BA_PANGVOICE,               100,		0,      INT_MAX, },
	//dançarina
	{ "d_DC_THROWARROW",                   &battle_config.d_DC_THROWARROW,               100,		0,      INT_MAX, },
	{ "d_DC_UGLYDANCE",                   &battle_config.d_DC_UGLYDANCE,               100,		0,      INT_MAX, },
	{ "d_DC_SCREAM",                   &battle_config.d_DC_SCREAM,               100,		0,      INT_MAX, },
	{ "d_DC_HUMMING",                   &battle_config.d_DC_HUMMING,               100,		0,      INT_MAX, },
	{ "d_DC_DONTFORGETME",                   &battle_config.d_DC_DONTFORGETME,               100,		0,      INT_MAX, },
	{ "d_DC_FORTUNEKISS",                   &battle_config.d_DC_FORTUNEKISS,               100,		0,      INT_MAX, },
	{ "d_DC_SERVICEFORYOU",                   &battle_config.d_DC_SERVICEFORYOU,               100,		0,      INT_MAX, },
	{ "d_DC_WINKCHARM",                   &battle_config.d_DC_WINKCHARM,               100,		0,      INT_MAX, },
	//Lorde
	{ "d_LK_SPIRALPIERCE",                   &battle_config.d_LK_SPIRALPIERCE,               100,		0,      INT_MAX, },
	{ "d_LK_HEADCRUSH",                   &battle_config.d_LK_HEADCRUSH,               100,		0,      INT_MAX, },
	{ "d_LK_JOINTBEAT",                   &battle_config.d_LK_JOINTBEAT,               100,		0,      INT_MAX, },
	{ "d_LK_FURY",                   &battle_config.d_LK_FURY,               100,		0,      INT_MAX, },
	//arquimago
	{ "d_HW_MAGICCRASHER",                   &battle_config.d_HW_MAGICCRASHER,               100,		0,      INT_MAX, },
	{ "d_HW_NAPALMVULCAN",                   &battle_config.d_HW_NAPALMVULCAN,               100,		0,      INT_MAX, },
	{ "d_HW_GANBANTEIN",                   &battle_config.d_HW_GANBANTEIN,               100,		0,      INT_MAX, },
	{ "d_HW_GRAVITATION",                   &battle_config.d_HW_GRAVITATION,               100,		0,      INT_MAX, },
	//paladino
	{ "d_SM_AUTOBERSERK",                   &battle_config.d_SM_AUTOBERSERK,               100,		0,      INT_MAX, },
	{ "d_PA_SACRIFICE",                   &battle_config.d_PA_SACRIFICE,               100,		0,      INT_MAX, },
	{ "d_PA_PRESSURE",                   &battle_config.d_PA_PRESSURE,               100,		0,      INT_MAX, },
	{ "d_PA_SHIELDCHAIN",                   &battle_config.d_PA_SHIELDCHAIN,               100,		0,      INT_MAX, },
	//mestre
	{ "d_CH_PALMSTRIKE",                   &battle_config.d_CH_PALMSTRIKE,               100,		0,      INT_MAX, },
	{ "d_CH_TIGERFIST",                   &battle_config.d_CH_TIGERFIST,               100,		0,      INT_MAX, },
	{ "d_CH_CHAINCRUSH",                   &battle_config.d_CH_CHAINCRUSH,               100,		0,      INT_MAX, },
	{ "d_CH_SOULCOLLECT",                   &battle_config.d_CH_SOULCOLLECT,               100,		0,      INT_MAX, },
	//professor
	{ "d_PF_SOULCHANGE",                   &battle_config.d_PF_SOULCHANGE,               100,		0,      INT_MAX, },
	{ "d_PF_SOULBURN",                   &battle_config.d_PF_SOULBURN,               100,		0,      INT_MAX, },
	{ "d_PF_DOUBLECASTING",                   &battle_config.d_PF_DOUBLECASTING,               100,		0,      INT_MAX, },
	{ "d_PF_MINDBREAKER",                   &battle_config.d_PF_MINDBREAKER,               100,		0,      INT_MAX, },
	{ "d_PF_SPIDERWEB",                   &battle_config.d_PF_SPIDERWEB,               100,		0,      INT_MAX, },
	//Algoz
	{ "d_ASC_BREAKER",                   &battle_config.d_ASC_BREAKER,               100,		0,      INT_MAX, },
	{ "d_ASC_METEORASSAULT",                   &battle_config.d_ASC_METEORASSAULT,               100,		0,      INT_MAX, },
	//Atirador de elite
	{ "d_SN_FALCONASSAULT",                   &battle_config.d_SN_FALCONASSAULT,               100,		0,      INT_MAX, },
	{ "d_SN_SHARPSHOOTING",                   &battle_config.d_SN_SHARPSHOOTING,               100,		0,      INT_MAX, },
	//Criador
	{ "d_CR_ALCHEMY",                   &battle_config.d_CR_ALCHEMY,               100,		0,      INT_MAX, },
	{ "d_CR_SYNTHESISPOTION",                   &battle_config.d_CR_SYNTHESISPOTION,               100,		0,      INT_MAX, },
	{ "d_CR_SLIMPITCHER",                   &battle_config.d_CR_SLIMPITCHER,               100,		0,      INT_MAX, },
	{ "d_CR_FULLPROTECTION",                   &battle_config.d_CR_FULLPROTECTION,               100,		0,      INT_MAX, },
	{ "d_CR_ACIDDEMONSTRATION",                   &battle_config.d_CR_ACIDDEMONSTRATION,               100,		0,      INT_MAX, },
	//menestrel e cigana
	{ "d_CG_MOONLIT",                   &battle_config.d_CG_MOONLIT,               100,		0,      INT_MAX, },
	{ "d_CG_MARIONETTE",                   &battle_config.d_CG_MARIONETTE,               100,		0,      INT_MAX, },
	{ "d_CG_HERMODE",                   &battle_config.d_CG_HERMODE,               100,		0,      INT_MAX, },
	{ "d_CG_TAROTCARD",                   &battle_config.d_CG_TAROTCARD,               100,		0,      INT_MAX, },
	{ "d_CG_ARROWVULCAN",                   &battle_config.d_CG_ARROWVULCAN,               100,		0,      INT_MAX, },
	//Desordeiro
	{ "d_ST_FULLSTRIP",                   &battle_config.d_ST_FULLSTRIP,               100,		0,      INT_MAX, },
	//Sumo Sacerdote
	{ "d_HP_MANARECHARGE",                   &battle_config.d_HP_MANARECHARGE,               100,		0,      INT_MAX, },
	//Mestre Ferreiro
	{ "d_WS_CARTTERMINATION",                   &battle_config.d_WS_CARTTERMINATION,               100,		0,      INT_MAX, },
	//taekwon
	{ "d_TK_STORMKICK",                   &battle_config.d_TK_STORMKICK,               100,		0,      INT_MAX, },
	{ "d_TK_DOWNKICK",                   &battle_config.d_TK_DOWNKICK,               100,		0,      INT_MAX, },
	{ "d_TK_TURNKICK",                   &battle_config.d_TK_TURNKICK,               100,		0,      INT_MAX, },
	{ "d_TK_JUMPKICK",                   &battle_config.d_TK_JUMPKICK,               100,		0,      INT_MAX, },
	{ "d_TK_POWER",                   &battle_config.d_TK_POWER,               100,		0,      INT_MAX, },
	{ "d_TK_HIGHJUMP",                   &battle_config.d_TK_HIGHJUMP,               100,		0,      INT_MAX, },
	//Espiritualista
	{ "d_SL_KAIZEL",                   &battle_config.d_SL_KAIZEL,               100,		0,      INT_MAX, },
	{ "d_SL_KAAHI",                   &battle_config.d_SL_KAAHI,               100,		0,      INT_MAX, },
	{ "d_SL_KAUPE",                   &battle_config.d_SL_KAUPE,               100,		0,      INT_MAX, },
	{ "d_SL_KAITE",                   &battle_config.d_SL_KAITE,               100,		0,      INT_MAX, },
	{ "d_SL_KAINA",                   &battle_config.d_SL_KAINA,               100,		0,      INT_MAX, },
	{ "d_SL_STIN",                   &battle_config.d_SL_STIN,               100,		0,      INT_MAX, },
	{ "d_SL_STUN",                   &battle_config.d_SL_STUN,               100,		0,      INT_MAX, },
	{ "d_SL_SMA",                   &battle_config.d_SL_SMA,               100,		0,      INT_MAX, },
	{ "d_SL_SWOO",                   &battle_config.d_SL_SWOO,               100,		0,      INT_MAX, },
	{ "d_SL_SKE",                   &battle_config.d_SL_SKE,               100,		0,      INT_MAX, },
	{ "d_SL_SKA",                   &battle_config.d_SL_SKA,               100,		0,      INT_MAX, },
	{ "d_SL_HIGH",                   &battle_config.d_SL_HIGH,               100,		0,      INT_MAX, },
	//Justiceiro
	{ "d_GS_TRIPLEACTION",                   &battle_config.d_GS_TRIPLEACTION,               100,		0,      INT_MAX, },
	{ "d_GS_BULLSEYE",                   &battle_config.d_GS_BULLSEYE,               100,		0,      INT_MAX, },
	{ "d_GS_MADNESSCANCEL",                   &battle_config.d_GS_BULLSEYE,               100,		0,      INT_MAX, },
	{ "d_GS_INCREASING",                   &battle_config.d_GS_INCREASING,               100,		0,      INT_MAX, },
	{ "d_GS_MAGICALBULLET",                   &battle_config.d_GS_MAGICALBULLET,               100,		0,      INT_MAX, },
	{ "d_GS_CRACKER",                   &battle_config.d_GS_CRACKER,               100,		0,      INT_MAX, },
	{ "d_GS_SINGLEACTION",                   &battle_config.d_GS_SINGLEACTION,               100,		0,      INT_MAX, },
	{ "d_GS_CHAINACTION",                   &battle_config.d_GS_CHAINACTION,               100,		0,      INT_MAX, },
	{ "d_GS_TRACKING",                   &battle_config.d_GS_TRACKING,               100,		0,      INT_MAX, },
	{ "d_GS_DISARM",                   &battle_config.d_GS_DISARM,               100,		0,      INT_MAX, },
	{ "d_GS_PIERCINGSHOT",                   &battle_config.d_GS_PIERCINGSHOT,               100,		0,      INT_MAX, },
	{ "d_GS_RAPIDSHOWER",                   &battle_config.d_GS_RAPIDSHOWER,               100,		0,      INT_MAX, },
	{ "d_GS_GATLINGFEVER",                   &battle_config.d_GS_GATLINGFEVER,               100,		0,      INT_MAX, },
	{ "d_GS_DUST",                   &battle_config.d_GS_DUST,               100,		0,      INT_MAX, },
	{ "d_GS_FULLBUSTER",                   &battle_config.d_GS_FULLBUSTER,               100,		0,      INT_MAX, },
	{ "d_GS_SPREADATTACK",                   &battle_config.d_GS_SPREADATTACK,               100,		0,      INT_MAX, },
	{ "d_GS_GROUNDDRIFT",                   &battle_config.d_GS_GROUNDDRIFT,               100,		0,      INT_MAX, },
	{ "d_GS_DESPERADO",                   &battle_config.d_GS_DESPERADO,               100,		0,      INT_MAX, },
	//Ninja
	{ "d_NJ_TOBIDOUGU",                   &battle_config.d_NJ_TOBIDOUGU,               100,		0,      INT_MAX, },
	{ "d_NJ_SYURIKEN",                   &battle_config.d_NJ_SYURIKEN,               100,		0,      INT_MAX, },
	{ "d_NJ_KUNAI",                   &battle_config.d_NJ_KUNAI,               100,		0,      INT_MAX, },
	{ "d_NJ_HUUMA",                   &battle_config.d_NJ_HUUMA,               100,		0,      INT_MAX, },
	{ "d_NJ_ZENYNAGE",                   &battle_config.d_NJ_ZENYNAGE,               100,		0,      INT_MAX, },
	{ "d_NJ_TATAMIGAESHI",                   &battle_config.d_NJ_TATAMIGAESHI,               100,		0,      INT_MAX, },
	{ "d_NJ_KASUMIKIRI",                   &battle_config.d_NJ_KASUMIKIRI,               100,		0,      INT_MAX, },
	{ "d_NJ_SHADOWJUMP",                   &battle_config.d_NJ_SHADOWJUMP,               100,		0,      INT_MAX, },
	{ "d_NJ_KIRIKAGE",                   &battle_config.d_NJ_KIRIKAGE,               100,		0,      INT_MAX, },
	{ "d_NJ_UTSUSEMI",                   &battle_config.d_NJ_UTSUSEMI,               100,		0,      INT_MAX, },
	{ "d_NJ_BUNSINJYUTSU",                   &battle_config.d_NJ_BUNSINJYUTSU,               100,		0,      INT_MAX, },
	{ "d_NJ_NINPOU",                   &battle_config.d_NJ_NINPOU,               100,		0,      INT_MAX, },
	{ "d_NJ_KOUENKA",                   &battle_config.d_NJ_KOUENKA,               100,		0,      INT_MAX, },
	{ "d_NJ_KAENSIN",                   &battle_config.d_NJ_KAENSIN,               100,		0,      INT_MAX, },
	{ "d_NJ_BAKUENRYU",                   &battle_config.d_NJ_BAKUENRYU,               100,		0,      INT_MAX, },
	{ "d_NJ_SUITON",                   &battle_config.d_NJ_SUITON,               100,		0,      INT_MAX, },
	{ "d_NJ_HYOUSYOURAKU",                   &battle_config.d_NJ_HYOUSYOURAKU,               100,		0,      INT_MAX, },
	{ "d_NJ_HUUJIN",                   &battle_config.d_NJ_HUUJIN,               100,		0,      INT_MAX, },
	{ "d_NJ_RAIGEKISAI",                   &battle_config.d_NJ_RAIGEKISAI,               100,		0,      INT_MAX, },
	{ "d_NJ_KAMAITACHI",                   &battle_config.d_NJ_KAMAITACHI,               100,		0,      INT_MAX, },
	{ "d_NJ_NEN",                   &battle_config.d_NJ_NEN,               100,		0,      INT_MAX, },
	{ "d_NJ_ISSEN",                   &battle_config.d_NJ_ISSEN,               100,		0,      INT_MAX, },
	{ "d_NJ_HYOUSENSOU",                   &battle_config.d_NJ_HYOUSENSOU,               100,		0,      INT_MAX, },
	//PEDRODKS
	//BOR SKILLS
	{ "d_BA_BAIKANOJUTSU",               &battle_config.d_BA_BAIKANOJUTSU,               100,		0,      INT_MAX, },
	{ "d_BA_PVERDE",                     &battle_config.d_BA_PVERDE,               100,		0,      INT_MAX, },
	{ "d_BA_PAMARELA",                   &battle_config.d_BA_PAMARELA,               100,		0,      INT_MAX, },
	{ "d_BA_PVERMELHA",                  &battle_config.d_BA_PVERMELHA,               100,		0,      INT_MAX, },
	{ "d_BOR_CHOUJI_01",                  &battle_config.d_BOR_CHOUJI_01,               100,		0,      INT_MAX, },
	{ "d_PC_PORTAO01",                   &battle_config.d_PC_PORTAO01,               100,		0,      INT_MAX, },
	{ "d_PC_PORTAO02",                   &battle_config.d_PC_PORTAO02,               100,		0,      INT_MAX, },
	{ "d_PC_PORTAO03",                   &battle_config.d_PC_PORTAO03,               100,		0,      INT_MAX, },
	{ "d_PC_PORTAO04",                   &battle_config.d_PC_PORTAO04,               100,		0,      INT_MAX, },
	{ "d_PC_PORTAO05",                   &battle_config.d_PC_PORTAO05,               100,		0,      INT_MAX, },
	{ "d_PC_PORTAO06",                   &battle_config.d_PC_PORTAO06,               100,		0,      INT_MAX, },
	{ "d_PC_PORTAO07",                   &battle_config.d_PC_PORTAO07,               100,		0,      INT_MAX, },
	{ "d_PC_PORTAO08",                   &battle_config.d_PC_PORTAO08,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYCACH01",               &battle_config.d_BOR_KUCHYCACH01,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYCACH02",               &battle_config.d_BOR_KUCHYCACH02,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYCACH03",               &battle_config.d_BOR_KUCHYCACH03,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYCOBRA01",              &battle_config.d_BOR_KUCHYCOBRA01,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYCOBRA02",              &battle_config.d_BOR_KUCHYCOBRA02,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYCOBRA03",              &battle_config.d_BOR_KUCHYCOBRA03,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYLESMA01",              &battle_config.d_BOR_KUCHYLESMA01,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYLESMA02",              &battle_config.d_BOR_KUCHYLESMA02,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYLESMA03",              &battle_config.d_BOR_KUCHYLESMA03,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYMACAC01",              &battle_config.d_BOR_KUCHYMACAC01,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYMACAC02",              &battle_config.d_BOR_KUCHYMACAC02,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYMACAC03",              &battle_config.d_BOR_KUCHYMACAC03,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYSAPO01",               &battle_config.d_BOR_KUCHYSAPO01,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYSAPO02",               &battle_config.d_BOR_KUCHYSAPO02,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYSAPO03",               &battle_config.d_BOR_KUCHYSAPO03,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYIBUSE01",              &battle_config.d_BOR_KUCHYIBUSE01,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYIBUSE02",              &battle_config.d_BOR_KUCHYIBUSE02,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYIBUSE03",              &battle_config.d_BOR_KUCHYIBUSE03,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYTAYUYA01",             &battle_config.d_BOR_KUCHYTAYUYA01,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYTAYUYA02",             &battle_config.d_BOR_KUCHYTAYUYA02,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYTAYUYA03",             &battle_config.d_BOR_KUCHYTAYUYA03,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYKUGUTSU01",            &battle_config.d_BOR_KUCHYKUGUTSU01,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYKUGUTSU02",            &battle_config.d_BOR_KUCHYKUGUTSU02,               100,		0,      INT_MAX, },
	{ "d_BOR_KUCHYKUGUTSU03",            &battle_config.d_BOR_KUCHYKUGUTSU03,               100,		0,      INT_MAX, },
	{ "d_BOR_BYAKUGAN",                  &battle_config.d_BOR_BYAKUGAN,               100,		0,      INT_MAX, },
	{ "d_BOR_ROKUJUYON",                 &battle_config.d_BOR_ROKUJUYON,               100,		0,      INT_MAX, },
	{ "d_BOR_8TRIG",                     &battle_config.d_BOR_8TRIG,               100,		0,      INT_MAX, },
	{ "d_BOR_HIJUTSU",                   &battle_config.d_BOR_HIJUTSU,               100,		0,      INT_MAX, },
	{ "d_BOR_SHARINT01",                 &battle_config.d_BOR_SHARINT01,               100,		0,      INT_MAX, },
	{ "d_BOR_SHARINT02",                 &battle_config.d_BOR_SHARINT02,               100,		0,      INT_MAX, },
	{ "d_BOR_SHARINT03",                 &battle_config.d_BOR_SHARINT03,               100,		0,      INT_MAX, },
	{ "d_BOR_SHAMSASUKE",                &battle_config.d_BOR_SHAMSASUKE,               100,		0,      INT_MAX, },
	{ "d_BOR_SHAMITACHI",                &battle_config.d_BOR_SHAMITACHI,               100,		0,      INT_MAX, },
	{ "d_BOR_SHAMOBITO",                 &battle_config.d_BOR_SHAMOBITO,               100,		0,      INT_MAX, },
	{ "d_BOR_TSUKUYOMI",                 &battle_config.d_BOR_TSUKUYOMI,               100,		0,      INT_MAX, },
	{ "d_BOR_SUSANOSASUKE",              &battle_config.d_BOR_SUSANOSASUKE,               100,		0,      INT_MAX, },
	{ "d_BOR_SHINCHU",                   &battle_config.d_BOR_SHINCHU,               100,		0,      INT_MAX, },
	{ "d_BOR_KOKUANGYO",                 &battle_config.d_BOR_KOKUANGYO,               100,		0,      INT_MAX, },
	{ "d_BOR_KANENOOTO",                 &battle_config.d_BOR_KANENOOTO,               100,		0,      INT_MAX, },
	{ "d_BOR_GENWAKU",                   &battle_config.d_BOR_GENWAKU,               100,		0,      INT_MAX, },
	{ "d_BOR_SHIBARI",                   &battle_config.d_BOR_SHIBARI,               100,		0,      INT_MAX, },
	{ "d_BOR_FCHAKRA",                   &battle_config.d_BOR_FCHAKRA,               100,		0,      INT_MAX, },
	{ "d_BOR_NEHAN",                     &battle_config.d_BOR_NEHAN,               100,		0,      INT_MAX, },
	{ "d_BOR_NARAKUMI",                  &battle_config.d_BOR_NARAKUMI,               100,		0,      INT_MAX, },
	{ "d_BOR_KATON01",                   &battle_config.d_BOR_KATON01,               100,		0,      INT_MAX, },
	{ "d_BOR_SUITON01",                  &battle_config.d_BOR_SUITON01,               100,		0,      INT_MAX, },
	{ "d_BOR_SUITON02",                  &battle_config.d_BOR_SUITON02,               100,		0,      INT_MAX, },
	{ "d_BOR_SUITON03",                  &battle_config.d_BOR_SUITON03,               100,		0,      INT_MAX, },
	{ "d_BOR_SUITON04",                  &battle_config.d_BOR_SUITON04,               100,		0,      INT_MAX, },
	{ "d_BOR_KATON02",                   &battle_config.d_BOR_KATON02,               100,		0,      INT_MAX, },
	{ "d_BOR_KATON03",                   &battle_config.d_BOR_KATON03,               100,		0,      INT_MAX, },
	{ "d_BOR_KATON04",                   &battle_config.d_BOR_KATON04,               100,		0,      INT_MAX, },
	{ "d_BOR_KATON05",                   &battle_config.d_BOR_KATON05,               100,		0,      INT_MAX, },
	{ "d_BOR_MEDICINA01",                &battle_config.d_BOR_MEDICINA01,               100,		0,      INT_MAX, },
	{ "d_BOR_TERREMOTO",                 &battle_config.d_BOR_TERREMOTO,               100,		0,      INT_MAX, },
	{ "d_BOR_RELAMPAGO",                 &battle_config.d_BOR_RELAMPAGO,               100,		0,      INT_MAX, },
	{ "d_BOR_RAITON_F",                  &battle_config.d_BOR_RAITON_F,               100,		0,      INT_MAX, },
	{ "d_BOR_SUITON05",                  &battle_config.d_BOR_SUITON05,               100,		0,      INT_MAX, },
	{ "d_BOR_DOTON01",                   &battle_config.d_BOR_DOTON01,               100,		0,      INT_MAX, },
	{ "d_BOR_NARA_03",                   &battle_config.d_BOR_NARA_03,               100,		0,      INT_MAX, },
	{ "d_BOR_NARA_04",                   &battle_config.d_BOR_NARA_04,               100,		0,      INT_MAX, },
	{ "d_BOR_NARA_05",                   &battle_config.d_BOR_NARA_05,               100,		0,      INT_MAX, },
	{ "d_BOR_NARA_06",                   &battle_config.d_BOR_NARA_06,               100,		0,      INT_MAX, },
	{ "d_BOR_BYAKUGAN_CONTROL",          &battle_config.d_BOR_BYAKUGAN_CONTROL,               100,		0,      INT_MAX, },
	{ "d_BOR_RAITON_F2",                 &battle_config.d_BOR_RAITON_F2,               100,		0,      INT_MAX, },
};
...
void battle_adjust_conf(void) {
	...
	if (battle_config.mvp_exp_reward_message) {
		ShowWarning("conf/map/battle/client.conf MVP EXP reward message está ativo mais requer cliente PACKETVER 2013-12-23 ou mais, desativando...\n");
	...
	if (battle_config.enable_refinery_ui == 1) {
		ShowWarning("conf/map/battle/universe.conf refinery ui está ativo mais requer cliente PACKETVER 2016-11-09 RagexeRE/2016-11-30 Ragexe or newer, desativando...\n");
		battle_config.enable_refinery_ui = 0;
	}

	if (battle_config.replace_refine_npcs == 1) {
		ShowWarning("conf/map/battle/universe.conf replace refine npcs está ativo mais requer cliente PACKETVER 2016-11-09 RagexeRE/2016-11-30 Ragexe or newer, desativando...\n");
...
//LEITURA DOS VALORES PARA UTILIZAR NAS FORMULAS DAS SKILLS DO BOR
static int battle_skill_config_read(const char* cfgName)
{
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		if (strcmpi(w1, "tsukuyomi_rate_luk") == 0) {
			sscanf(w2, "%lf", &value);
			battle_config.tsukuyomi_rate_luk = value;
			if (!value) {
				ShowError("tsukuyomi_rate_luk especificado '%1f' não existe no cache.\n", value);
			}
			ShowConf("Sistema Genjutsu - Tsukuyomi Rate: '"CL_CYAN"%1f"CL_RESET"' no arquivo '"CL_WHITE"%s"CL_RESET"'.\n", value, cfgName);
		}
		else if (strcmpi(w1, "korishinchu_rate_luk") == 0) {
			sscanf(w2, "%lf", &value);
			battle_config.korishinchu_rate_luk = value;
			if (!value) {
				ShowError("korishinchu_rate_luk especificado '%1f' não existe no cache.\n", value);
			}
			ShowConf("Sistema Genjutsu - Kori Shinchu Rate: '"CL_CYAN"%1f"CL_RESET"' no arquivo '"CL_WHITE"%s"CL_RESET"'.\n", value, cfgName);
		}
		else if (strcmpi(w1, "kokuangyo_rate_luk") == 0) {
			sscanf(w2, "%lf", &value);
			battle_config.kokuangyo_rate_luk = value;
			if (!value) {
				ShowError("kokuangyo_rate_luk especificado '%1f' não existe no cache.\n", value);
			}
			ShowConf("Sistema Genjutsu - Kokuangyo Rate: '"CL_CYAN"%1f"CL_RESET"' no arquivo '"CL_WHITE"%s"CL_RESET"'.\n", value, cfgName);
		}
		else if (strcmpi(w1, "kanenooto_rate_luk") == 0) {
			sscanf(w2, "%lf", &value);
			battle_config.kanenooto_rate_luk = value;
			if (!value) {
				ShowError("kanenooto_rate_luk especificado '%1f' não existe no cache.\n", value);
			}
			ShowConf("Sistema Genjutsu - Kane no Oto: '"CL_CYAN"%1f"CL_RESET"' no arquivo '"CL_WHITE"%s"CL_RESET"'.\n", value, cfgName);
		}
		else if (strcmpi(w1, "genwaku_rate_luk") == 0) {
			sscanf(w2, "%lf", &value);
			battle_config.genwaku_rate_luk = value;
			if (!value) {
				ShowError("genwaku_rate_luk especificado '%1f' não existe no cache.\n", value);
			}
			ShowConf("Sistema Genjutsu - Genwaku Rate: '"CL_CYAN"%1f"CL_RESET"' no arquivo '"CL_WHITE"%s"CL_RESET"'.\n", value, cfgName);
		}
		else if (strcmpi(w1, "shibari_rate_luk") == 0) {
			sscanf(w2, "%lf", &value);
			battle_config.shibari_rate_luk = value;
			if (!value) {
				ShowError("shibari_rate_luk especificado '%1f' não existe no cache.\n", value);
			}
			ShowConf("Sistema Genjutsu - Shibari Rate: '"CL_CYAN"%1f"CL_RESET"' no arquivo '"CL_WHITE"%s"CL_RESET"'.\n", value, cfgName);
		}
		else if (strcmpi(w1, "fchakra_rate_luk") == 0) {
			sscanf(w2, "%lf", &value);
			battle_config.fchakra_rate_luk = value;
			if (!value) {
				ShowError("fchakra_rate_luk especificado '%1f' não existe no cache.\n", value);
			}
			ShowConf("Sistema Genjutsu - Fantasmas Rate: '"CL_CYAN"%1f"CL_RESET"' no arquivo '"CL_WHITE"%s"CL_RESET"'.\n", value, cfgName);
		}
		else if (strcmpi(w1, "nehanshoja_rate_luk") == 0) {
			sscanf(w2, "%lf", &value);
			battle_config.nehanshoja_rate_luk = value;
			if (!value) {
				ShowError("nehanshoja_rate_luk especificado '%1f' não existe no cache.\n", value);
			}
			ShowConf("Sistema Genjutsu - Nehan Shoja Rate: '"CL_CYAN"%1f"CL_RESET"' no arquivo '"CL_WHITE"%s"CL_RESET"'.\n", value, cfgName);
		}
		else if (strcmpi(w1, "narakumi_rate_luk") == 0) {
			sscanf(w2, "%lf", &value);
			battle_config.narakumi_rate_luk = value;
			if (!value) {
				ShowError("narakumi_rate_luk especificado '%1f' não existe no cache.\n", value);
			}
			ShowConf("Sistema Genjutsu - Narakumi Rate: '"CL_CYAN"%1f"CL_RESET"' no arquivo '"CL_WHITE"%s"CL_RESET"'.\n", value, cfgName);
		}
		else if (strcmpi(w1, "kuchy_cach_id01") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_cach_id01 = variavel;
		}
		else if (strcmpi(w1, "kuchy_cach_id02") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_cach_id02 = variavel;
		}
		else if (strcmpi(w1, "kuchy_cach_id03") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_cach_id03 = variavel;
		}
		else if (strcmpi(w1, "kuchy_cobr_id01") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_cobr_id01 = variavel;
		}
		else if (strcmpi(w1, "kuchy_cobr_id02") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_cobr_id02 = variavel;
		}
		else if (strcmpi(w1, "kuchy_cobr_id03") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_cobr_id03 = variavel;
		}
		else if (strcmpi(w1, "kuchy_lesm_id01") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_lesm_id01 = variavel;
		}
		else if (strcmpi(w1, "kuchy_lesm_id02") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_lesm_id02 = variavel;
		}
		else if (strcmpi(w1, "kuchy_lesm_id03") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_lesm_id03 = variavel;
		}
		else if (strcmpi(w1, "kuchy_maca_id01") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_maca_id01 = variavel;
		}
		else if (strcmpi(w1, "kuchy_maca_id02") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_maca_id02 = variavel;
		}
		else if (strcmpi(w1, "kuchy_maca_id03") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_maca_id03 = variavel;
		}
		else if (strcmpi(w1, "kuchy_sapo_id01") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_sapo_id01 = variavel;
		}
		else if (strcmpi(w1, "kuchy_sapo_id02") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_sapo_id02 = variavel;
		}
		else if (strcmpi(w1, "kuchy_sapo_id03") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_sapo_id03 = variavel;
		}
		else if (strcmpi(w1, "kuchy_ibus_id01") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_ibus_id01 = variavel;
		}
		else if (strcmpi(w1, "kuchy_ibus_id02") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_ibus_id02 = variavel;
		}
		else if (strcmpi(w1, "kuchy_ibus_id03") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_ibus_id03 = variavel;
		}
		else if (strcmpi(w1, "kuchy_tayu_id01") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_tayu_id01 = variavel;
		}
		else if (strcmpi(w1, "kuchy_tayu_id02") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_tayu_id02 = variavel;
		}
		else if (strcmpi(w1, "kuchy_tayu_id03") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_tayu_id03 = variavel;
		}
		else if (strcmpi(w1, "kuchy_kugu_id01") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_kugu_id01 = variavel;
		}
		else if (strcmpi(w1, "kuchy_kugu_id02") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_kugu_id02 = variavel;
		}
		else if (strcmpi(w1, "kuchy_kugu_id03") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kuchy_kugu_id03 = variavel;
		}
		else if (strcmpi(w1, "katon_01_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.katon_01_active = variavel;
		}
		else if (strcmpi(w1, "katon_01_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.katon_01_chance = variavel;
		}
		else if (strcmpi(w1, "katon_02_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.katon_02_active = variavel;
		}
		else if (strcmpi(w1, "katon_02_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.katon_02_chance = variavel;
		}
		else if (strcmpi(w1, "katon_03_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.katon_03_active = variavel;
		}
		else if (strcmpi(w1, "katon_03_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.katon_03_chance = variavel;
		}
		else if (strcmpi(w1, "katon_04_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.katon_04_active = variavel;
		}
		else if (strcmpi(w1, "katon_04_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.katon_04_chance = variavel;
		}
		else if (strcmpi(w1, "katon_05_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.katon_05_active = variavel;
		}
		else if (strcmpi(w1, "katon_05_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.katon_05_chance = variavel;
		}
		else if (strcmpi(w1, "suiton_01_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.suiton_01_active = variavel;
		}
		else if (strcmpi(w1, "suiton_01_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.suiton_01_chance = variavel;
		}
		else if (strcmpi(w1, "suiton_02_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.suiton_02_active = variavel;
		}
		else if (strcmpi(w1, "suiton_02_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.suiton_02_chance = variavel;
		}
		else if (strcmpi(w1, "suiton_03_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.suiton_03_active = variavel;
		}
		else if (strcmpi(w1, "suiton_03_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.suiton_03_chance = variavel;
		}
		else if (strcmpi(w1, "suiton_04_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.suiton_04_active = variavel;
		}
		else if (strcmpi(w1, "suiton_04_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.suiton_04_chance = variavel;
		}
		else if (strcmpi(w1, "suiton_05_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.suiton_05_active = variavel;
		}
		else if (strcmpi(w1, "suiton_05_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.suiton_05_chance = variavel;
		}
		else if (strcmpi(w1, "raiton_01_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_01_active = variavel;
		}
		else if (strcmpi(w1, "raiton_01_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_01_chance = variavel;
		}
		else if (strcmpi(w1, "raiton_02_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_02_active = variavel;
		}
		else if (strcmpi(w1, "raiton_02_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_02_chance = variavel;
		}
		else if (strcmpi(w1, "raiton_03_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_03_active = variavel;
		}
		else if (strcmpi(w1, "raiton_03_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_03_chance = variavel;
		}
		else if (strcmpi(w1, "raiton_04_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_04_active = variavel;
		}
		else if (strcmpi(w1, "raiton_04_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_04_chance = variavel;
		}
		else if (strcmpi(w1, "raiton_05_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_05_active = variavel;
		}
		else if (strcmpi(w1, "raiton_05_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_05_chance = variavel;
		}
		else if (strcmpi(w1, "raiton_06_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_06_active = variavel;
		}
		else if (strcmpi(w1, "raiton_06_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_06_chance = variavel;
		}
		else if (strcmpi(w1, "raiton_07_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_07_active = variavel;
		}
		else if (strcmpi(w1, "raiton_07_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_07_chance = variavel;
		}
		else if (strcmpi(w1, "raiton_08_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_08_active = variavel;
		}
		else if (strcmpi(w1, "raiton_08_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.raiton_08_chance = variavel;
		}
		else if (strcmpi(w1, "doton_01_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doton_01_active = variavel;
		}
		else if (strcmpi(w1, "doton_01_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doton_01_chance = variavel;
		}
		else if (strcmpi(w1, "doton_02_active") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doton_02_active = variavel;
		}
		else if (strcmpi(w1, "doton_02_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doton_02_chance = variavel;
		}
		else if (strcmpi(w1, "queimadura_balance") == 0) {
			sscanf(w2, "%lf", &value);
			battle_config.queimadura_balance = value;
		}
		else if (strcmpi(w1, "doujutsu_sharigan_item_nv1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doujutsu_sharigan_item_nv1 = variavel;
		}
		else if (strcmpi(w1, "doujutsu_sharigan_item_nv2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doujutsu_sharigan_item_nv2 = variavel;
		}
		else if (strcmpi(w1, "doujutsu_sharigan_item_nv3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doujutsu_sharigan_item_nv3 = variavel;
		}
		else if (strcmpi(w1, "doujutsu_sharigan_item_nv4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doujutsu_sharigan_item_nv4 = variavel;
		}
		else if (strcmpi(w1, "doujutsu_byakugan_item_nv1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doujutsu_byakugan_item_nv1 = variavel;
		}
		else if (strcmpi(w1, "doujutsu_byakugan_item_nv2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doujutsu_byakugan_item_nv2 = variavel;
		}
		else if (strcmpi(w1, "doujutsu_byakugan_item_nv3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doujutsu_byakugan_item_nv3 = variavel;
		}
		else if (strcmpi(w1, "doujutsu_byakugan_item_nv4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doujutsu_byakugan_item_nv4 = variavel;
		}
		else if (strcmpi(w1, "hidden_hachimon_item_nv1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hidden_hachimon_item_nv1 = variavel;
		}
		else if (strcmpi(w1, "hidden_hachimon_item_nv2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hidden_hachimon_item_nv2 = variavel;
		}
		else if (strcmpi(w1, "hidden_hachimon_item_nv3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hidden_hachimon_item_nv3 = variavel;
		}
		else if (strcmpi(w1, "hidden_hachimon_item_nv4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hidden_hachimon_item_nv4 = variavel;
		}
		else if (strcmpi(w1, "hidden_baika_item_nv1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hidden_baika_item_nv1 = variavel;
		}
		else if (strcmpi(w1, "hidden_baika_item_nv2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hidden_baika_item_nv2 = variavel;
		}
		else if (strcmpi(w1, "hidden_baika_item_nv3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hidden_baika_item_nv3 = variavel;
		}
		else if (strcmpi(w1, "hidden_baika_item_nv4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hidden_baika_item_nv4 = variavel;
		}
		else if (strcmpi(w1, "hidden_sombras_item_nv1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hidden_sombras_item_nv1 = variavel;
		}
		else if (strcmpi(w1, "hidden_sombras_item_nv2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hidden_sombras_item_nv2 = variavel;
		}
		else if (strcmpi(w1, "hidden_sombras_item_nv3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hidden_sombras_item_nv3 = variavel;
		}
		else if (strcmpi(w1, "hidden_sombras_item_nv4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hidden_sombras_item_nv4 = variavel;
		}
		else if (strcmpi(w1, "kg_hyoton_item_nv1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kg_hyoton_item_nv1 = variavel;
		}
		else if (strcmpi(w1, "kg_hyoton_item_nv2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kg_hyoton_item_nv2 = variavel;
		}
		else if (strcmpi(w1, "kg_hyoton_item_nv3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kg_hyoton_item_nv3 = variavel;
		}
		else if (strcmpi(w1, "kg_hyoton_item_nv4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.kg_hyoton_item_nv4 = variavel;
		}
		else if (strcmpi(w1, "refine_01_exp_nv_1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_01_exp_nv_1 = variavel;
		}
		else if (strcmpi(w1, "refine_02_exp_nv_1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_02_exp_nv_1 = variavel;
		}
		else if (strcmpi(w1, "refine_03_exp_nv_1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_03_exp_nv_1 = variavel;
		}
		else if (strcmpi(w1, "refine_04_exp_nv_1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_04_exp_nv_1 = variavel;
		}
		else if (strcmpi(w1, "refine_05_exp_nv_1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_05_exp_nv_1 = variavel;
		}
		else if (strcmpi(w1, "refine_06_exp_nv_1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_06_exp_nv_1 = variavel;
		}
		else if (strcmpi(w1, "refine_07_exp_nv_1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_07_exp_nv_1 = variavel;
		}
		else if (strcmpi(w1, "refine_08_exp_nv_1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_08_exp_nv_1 = variavel;
		}
		else if (strcmpi(w1, "refine_09_exp_nv_1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_09_exp_nv_1 = variavel;
		}
		else if (strcmpi(w1, "refine_10_exp_nv_1") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_10_exp_nv_1 = variavel;
		}
		else if (strcmpi(w1, "refine_01_exp_nv_2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_01_exp_nv_2 = variavel;
		}
		else if (strcmpi(w1, "refine_02_exp_nv_2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_02_exp_nv_2 = variavel;
		}
		else if (strcmpi(w1, "refine_03_exp_nv_2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_03_exp_nv_2 = variavel;
		}
		else if (strcmpi(w1, "refine_04_exp_nv_2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_04_exp_nv_2 = variavel;
		}
		else if (strcmpi(w1, "refine_05_exp_nv_2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_05_exp_nv_2 = variavel;
		}
		else if (strcmpi(w1, "refine_06_exp_nv_2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_06_exp_nv_2 = variavel;
		}
		else if (strcmpi(w1, "refine_07_exp_nv_2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_07_exp_nv_2 = variavel;
		}
		else if (strcmpi(w1, "refine_08_exp_nv_2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_08_exp_nv_2 = variavel;
		}
		else if (strcmpi(w1, "refine_09_exp_nv_2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_09_exp_nv_2 = variavel;
		}
		else if (strcmpi(w1, "refine_10_exp_nv_2") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_10_exp_nv_2 = variavel;
		}
		else if (strcmpi(w1, "refine_01_exp_nv_3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_01_exp_nv_3 = variavel;
		}
		else if (strcmpi(w1, "refine_02_exp_nv_3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_02_exp_nv_3 = variavel;
		}
		else if (strcmpi(w1, "refine_03_exp_nv_3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_03_exp_nv_3 = variavel;
		}
		else if (strcmpi(w1, "refine_04_exp_nv_3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_04_exp_nv_3 = variavel;
		}
		else if (strcmpi(w1, "refine_05_exp_nv_3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_05_exp_nv_3 = variavel;
		}
		else if (strcmpi(w1, "refine_06_exp_nv_3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_06_exp_nv_3 = variavel;
		}
		else if (strcmpi(w1, "refine_07_exp_nv_3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_07_exp_nv_3 = variavel;
		}
		else if (strcmpi(w1, "refine_08_exp_nv_3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_08_exp_nv_3 = variavel;
		}
		else if (strcmpi(w1, "refine_09_exp_nv_3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_09_exp_nv_3 = variavel;
		}
		else if (strcmpi(w1, "refine_10_exp_nv_3") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_10_exp_nv_3 = variavel;
		}
		else if (strcmpi(w1, "refine_01_exp_nv_4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_01_exp_nv_4 = variavel;
		}
		else if (strcmpi(w1, "refine_02_exp_nv_4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_02_exp_nv_4 = variavel;
		}
		else if (strcmpi(w1, "refine_03_exp_nv_4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_03_exp_nv_4 = variavel;
		}
		else if (strcmpi(w1, "refine_04_exp_nv_4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_04_exp_nv_4 = variavel;
		}
		else if (strcmpi(w1, "refine_05_exp_nv_4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_05_exp_nv_4 = variavel;
		}
		else if (strcmpi(w1, "refine_06_exp_nv_4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_06_exp_nv_4 = variavel;
		}
		else if (strcmpi(w1, "refine_07_exp_nv_4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_07_exp_nv_4 = variavel;
		}
		else if (strcmpi(w1, "refine_08_exp_nv_4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_08_exp_nv_4 = variavel;
		}
		else if (strcmpi(w1, "refine_09_exp_nv_4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_09_exp_nv_4 = variavel;
		}
		else if (strcmpi(w1, "refine_10_exp_nv_4") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_10_exp_nv_4 = variavel;
		}
		else if (strcmpi(w1, "refine_item_protect") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_item_protect = variavel;
		}
		else if (strcmpi(w1, "refine_01_item_protect_quant") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_01_item_protect_quant = variavel;
		}
		else if (strcmpi(w1, "refine_02_item_protect_quant") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_02_item_protect_quant = variavel;
		}
		else if (strcmpi(w1, "refine_03_item_protect_quant") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_03_item_protect_quant = variavel;
		}
		else if (strcmpi(w1, "refine_04_item_protect_quant") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_04_item_protect_quant = variavel;
		}
		else if (strcmpi(w1, "refine_05_item_protect_quant") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_05_item_protect_quant = variavel;
		}
		else if (strcmpi(w1, "refine_06_item_protect_quant") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_06_item_protect_quant = variavel;
		}
		else if (strcmpi(w1, "refine_07_item_protect_quant") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_07_item_protect_quant = variavel;
		}
		else if (strcmpi(w1, "refine_08_item_protect_quant") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_08_item_protect_quant = variavel;
		}
		else if (strcmpi(w1, "refine_09_item_protect_quant") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_09_item_protect_quant = variavel;
		}
		else if (strcmpi(w1, "refine_10_item_protect_quant") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_10_item_protect_quant = variavel;
		}
		else if (strcmpi(w1, "refine_01_value") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_01_value = variavel;
		}
		else if (strcmpi(w1, "refine_02_value") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_02_value = variavel;
		}
		else if (strcmpi(w1, "refine_03_value") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_03_value = variavel;
		}
		else if (strcmpi(w1, "refine_04_value") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_04_value = variavel;
		}
		else if (strcmpi(w1, "refine_05_value") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_05_value = variavel;
		}
		else if (strcmpi(w1, "refine_06_value") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_06_value = variavel;
		}
		else if (strcmpi(w1, "refine_07_value") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_07_value = variavel;
		}
		else if (strcmpi(w1, "refine_08_value") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_08_value = variavel;
		}
		else if (strcmpi(w1, "refine_09_value") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_09_value = variavel;
		}
		else if (strcmpi(w1, "refine_10_value") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.refine_10_value = variavel;
		}
		else if (strcmpi(w1, "byak_control") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.byak_control = variavel;
		}
		else if (strcmpi(w1, "prem_diar_vip") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.prem_diar_vip = variavel;
		}
		else if (strcmpi(w1, "sang_mode") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.sang_mode = variavel;
		}
		else if (strcmpi(w1, "sang_hp") == 0) {
			sscanf(w2, "%lf", &value);
			battle_config.sang_hp = value;
			if (!value) {
				ShowError("sang_hp: Valor especificado '%1f' não existe no cache.\n", value);
			}
		}
		else if (strcmpi(w1, "sang_time") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.sang_time = variavel;
		}
		else if (strcmpi(w1, "lotus_mode") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_mode = variavel;
		}
		else if (strcmpi(w1, "lotus_4_type") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_4_type = variavel;
		}
		else if (strcmpi(w1, "lotus_4_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_4_chance = variavel;
		}
		else if (strcmpi(w1, "lotus_8_type") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_8_type = variavel;
		}
		else if (strcmpi(w1, "lotus_8_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_8_chance = variavel;
		}
		else if (strcmpi(w1, "lotus_16_type") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_16_type = variavel;
		}
		else if (strcmpi(w1, "lotus_16_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_16_chance = variavel;
		}
		else if (strcmpi(w1, "lotus_32_type") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_32_type = variavel;
		}
		else if (strcmpi(w1, "lotus_32_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_32_chance = variavel;
		}
		else if (strcmpi(w1, "lotus_64_type") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_64_type = variavel;
		}
		else if (strcmpi(w1, "lotus_64_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_64_chance = variavel;
		}
		else if (strcmpi(w1, "lotus_64_coma") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_64_coma = variavel;
		}
		else if (strcmpi(w1, "lotus_128_type") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_128_type = variavel;
		}
		else if (strcmpi(w1, "lotus_128_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_128_chance = variavel;
		}
		else if (strcmpi(w1, "lotus_128_coma") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.lotus_128_coma = variavel;
		}
		else if (strcmpi(w1, "gama_01_hit_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.gama_01_hit_chance = variavel;
		}
		else if (strcmpi(w1, "gp01_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.gp01_chance = variavel;
		}
		else if (strcmpi(w1, "gama_02_hit_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.gama_02_hit_chance = variavel;
		}
		else if (strcmpi(w1, "gp02_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.gp02_chance = variavel;
		}
		else if (strcmpi(w1, "gp02_time") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.gp02_time = variavel;
		}
		else if (strcmpi(w1, "hebi_01_hit_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hebi_01_hit_chance = variavel;
		}
		else if (strcmpi(w1, "hp01_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hp01_chance = variavel;
		}
		else if (strcmpi(w1, "hp01_time") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hp01_time = variavel;
		}
		else if (strcmpi(w1, "hebi_02_hit_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hebi_02_hit_chance = variavel;
		}
		else if (strcmpi(w1, "hp02_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hp02_chance = variavel;
		}
		else if (strcmpi(w1, "hp02_time") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hp02_time = variavel;
		}
		else if (strcmpi(w1, "hp02_agi_rate") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hp02_agi_rate = variavel;
		}
		else if (strcmpi(w1, "hp02_agi_time") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.hp02_agi_time = variavel;
		}
		else if (strcmpi(w1, "bor_atk_base_limit") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bor_atk_base_limit = variavel;
		}
		else if (strcmpi(w1, "chibaku_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.chibaku_chance = variavel;
		}
		else if (strcmpi(w1, "gen_01_chance") == 0) {
			sscanf(w2, "%lf", &value);
			battle_config.gen_01_chance = value;
			if (!value) {
				ShowError("gen_01_chance especificado '%1f' não existe no cache.\n", value);
			}
			ShowConf("Sistema Genjutsu - Aparencia Ilusoria Chance: '"CL_CYAN"%1f"CL_RESET"' no arquivo '"CL_WHITE"%s"CL_RESET"'.\n", value, cfgName);
		}
		else if (strcmpi(w1, "gen_02_chance") == 0) {
			sscanf(w2, "%lf", &value);
			battle_config.gen_02_chance = value;
			if (!value) {
				ShowError("gen_02_chance especificado '%1f' não existe no cache.\n", value);
			}
			ShowConf("Sistema Genjutsu - Paralisia Chance: '"CL_CYAN"%1f"CL_RESET"' no arquivo '"CL_WHITE"%s"CL_RESET"'.\n", value, cfgName);
		}
		else if (strcmpi(w1, "amat_fogo_quant") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.amat_fogo_quant = variavel;
		}
		else if (strcmpi(w1, "nara_01_block") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.nara_01_block = variavel;
		}
		else if (strcmpi(w1, "nara_02_block") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.nara_02_block = variavel;
		}
		else if (strcmpi(w1, "nara_03_block") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.nara_03_block = variavel;
		}
			else if (strcmpi(w1, "ranked_system_ativado") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.ranked_system_ativado = variavel;
		}
		else if (strcmpi(w1, "point_rank_e") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.point_rank_e = variavel;
		}
		else if (strcmpi(w1, "point_rank_d") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.point_rank_d = variavel;
		}
		else if (strcmpi(w1, "point_rank_c") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.point_rank_c = variavel;
		}
		else if (strcmpi(w1, "point_rank_b") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.point_rank_b = variavel;
		}
		else if (strcmpi(w1, "point_rank_a") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.point_rank_a = variavel;
		}
		else if (strcmpi(w1, "point_rank_s") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.point_rank_s = variavel;
		}
		else if (strcmpi(w1, "rank_e_mata_e") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_e_mata_e = variavel;
		}
		else if (strcmpi(w1, "rank_e_mata_d") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_e_mata_d = variavel;
		}
		else if (strcmpi(w1, "rank_e_mata_c") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_e_mata_c = variavel;
		}
		else if (strcmpi(w1, "rank_e_mata_b") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_e_mata_b = variavel;
		}
		else if (strcmpi(w1, "rank_e_mata_a") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_e_mata_a = variavel;
		}
		else if (strcmpi(w1, "rank_e_mata_s") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_e_mata_s = variavel;
		}
		else if (strcmpi(w1, "rank_e_morre_e") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_e_morre_e = variavel;
		}
		else if (strcmpi(w1, "rank_e_morre_d") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_e_morre_d = variavel;
		}
		else if (strcmpi(w1, "rank_e_morre_c") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_e_morre_c = variavel;
		}
		else if (strcmpi(w1, "rank_e_morre_b") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_e_morre_b = variavel;
		}
		else if (strcmpi(w1, "rank_e_morre_a") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_e_morre_a = variavel;
		}
		else if (strcmpi(w1, "rank_e_morre_s") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_e_morre_s = variavel;
		}
		else if (strcmpi(w1, "rank_d_mata_e") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_d_mata_e = variavel;
		}
		else if (strcmpi(w1, "rank_d_mata_d") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_d_mata_d = variavel;
		}
		else if (strcmpi(w1, "rank_d_mata_c") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_d_mata_c = variavel;
		}
		else if (strcmpi(w1, "rank_d_mata_b") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_d_mata_b = variavel;
		}
		else if (strcmpi(w1, "rank_d_mata_a") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_d_mata_a = variavel;
		}
		else if (strcmpi(w1, "rank_d_mata_s") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_d_mata_s = variavel;
		}
		else if (strcmpi(w1, "rank_d_morre_e") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_d_morre_e = variavel;
		}
		else if (strcmpi(w1, "rank_d_morre_d") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_d_morre_d = variavel;
		}
		else if (strcmpi(w1, "rank_d_morre_c") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_d_morre_c = variavel;
		}
		else if (strcmpi(w1, "rank_d_morre_b") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_d_morre_b = variavel;
		}
		else if (strcmpi(w1, "rank_d_morre_a") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_d_morre_a = variavel;
		}
		else if (strcmpi(w1, "rank_d_morre_s") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_d_morre_s = variavel;
		}
		else if (strcmpi(w1, "rank_c_mata_e") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_c_mata_e = variavel;
		}
		else if (strcmpi(w1, "rank_c_mata_d") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_c_mata_d = variavel;
		}
		else if (strcmpi(w1, "rank_c_mata_c") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_c_mata_c = variavel;
		}
		else if (strcmpi(w1, "rank_c_mata_b") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_c_mata_b = variavel;
		}
		else if (strcmpi(w1, "rank_c_mata_a") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_c_mata_a = variavel;
		}
		else if (strcmpi(w1, "rank_c_mata_s") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_c_mata_s = variavel;
		}
		else if (strcmpi(w1, "rank_c_morre_e") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_c_morre_e = variavel;
		}
		else if (strcmpi(w1, "rank_c_morre_d") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_c_morre_d = variavel;
		}
		else if (strcmpi(w1, "rank_c_morre_c") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_c_morre_c = variavel;
		}
		else if (strcmpi(w1, "rank_c_morre_b") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_c_morre_b = variavel;
		}
		else if (strcmpi(w1, "rank_c_morre_a") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_c_morre_a = variavel;
		}
		else if (strcmpi(w1, "rank_c_morre_s") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_c_morre_s = variavel;
		}
		else if (strcmpi(w1, "rank_b_mata_e") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_b_mata_e = variavel;
		}
		else if (strcmpi(w1, "rank_b_mata_d") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_b_mata_d = variavel;
		}
		else if (strcmpi(w1, "rank_b_mata_c") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_b_mata_c = variavel;
		}
		else if (strcmpi(w1, "rank_b_mata_b") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_b_mata_b = variavel;
		}
		else if (strcmpi(w1, "rank_b_mata_a") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_b_mata_a = variavel;
		}
		else if (strcmpi(w1, "rank_b_mata_s") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_b_mata_s = variavel;
		}
		else if (strcmpi(w1, "rank_b_morre_e") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_b_morre_e = variavel;
		}
		else if (strcmpi(w1, "rank_b_morre_d") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_b_morre_d = variavel;
		}
		else if (strcmpi(w1, "rank_b_morre_c") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_b_morre_c = variavel;
		}
		else if (strcmpi(w1, "rank_b_morre_b") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_b_morre_b = variavel;
		}
		else if (strcmpi(w1, "rank_b_morre_a") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_b_morre_a = variavel;
		}
		else if (strcmpi(w1, "rank_b_morre_s") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_b_morre_s = variavel;
		}
		else if (strcmpi(w1, "rank_a_mata_e") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_a_mata_e = variavel;
		}
		else if (strcmpi(w1, "rank_a_mata_d") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_a_mata_d = variavel;
		}
		else if (strcmpi(w1, "rank_a_mata_c") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_a_mata_c = variavel;
		}
		else if (strcmpi(w1, "rank_a_mata_b") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_a_mata_b = variavel;
		}
		else if (strcmpi(w1, "rank_a_mata_a") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_a_mata_a = variavel;
		}
		else if (strcmpi(w1, "rank_a_mata_s") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_a_mata_s = variavel;
		}
		else if (strcmpi(w1, "rank_a_morre_e") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_a_morre_e = variavel;
		}
		else if (strcmpi(w1, "rank_a_morre_d") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_a_morre_d = variavel;
		}
		else if (strcmpi(w1, "rank_a_morre_c") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_a_morre_c = variavel;
		}
		else if (strcmpi(w1, "rank_a_morre_b") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_a_morre_b = variavel;
		}
		else if (strcmpi(w1, "rank_a_morre_a") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_a_morre_a = variavel;
		}
		else if (strcmpi(w1, "rank_a_morre_s") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_a_morre_s = variavel;
		}
		else if (strcmpi(w1, "rank_s_mata_e") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_s_mata_e = variavel;
		}
		else if (strcmpi(w1, "rank_s_mata_d") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_s_mata_d = variavel;
		}
		else if (strcmpi(w1, "rank_s_mata_c") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_s_mata_c = variavel;
		}
		else if (strcmpi(w1, "rank_s_mata_b") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_s_mata_b = variavel;
		}
		else if (strcmpi(w1, "rank_s_mata_a") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_s_mata_a = variavel;
		}
		else if (strcmpi(w1, "rank_s_mata_s") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_s_mata_s = variavel;
		}
		else if (strcmpi(w1, "rank_s_morre_e") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_s_morre_e = variavel;
		}
		else if (strcmpi(w1, "rank_s_morre_d") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_s_morre_d = variavel;
		}
		else if (strcmpi(w1, "rank_s_morre_c") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_s_morre_c = variavel;
		}
		else if (strcmpi(w1, "rank_s_morre_b") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_s_morre_b = variavel;
		}
		else if (strcmpi(w1, "rank_s_morre_a") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_s_morre_a = variavel;
		}
		else if (strcmpi(w1, "rank_s_morre_s") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.rank_s_morre_s = variavel;
		}
		else if (strcmpi(w1, "weapon_exp_lv1_point") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.weapon_exp_lv_point[0] = variavel;
		}
		else if (strcmpi(w1, "weapon_exp_lv2_point") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.weapon_exp_lv_point[1] = variavel;
			if (battle_config.weapon_exp_lv_point[1] < battle_config.weapon_exp_lv_point[0]) {
				ShowError("weapon_exp_lv2_point valor especificado '%d' é menor do que weapon_exp_lv1_point.\n", variavel);
			}
		}
		else if (strcmpi(w1, "weapon_exp_lv3_point") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.weapon_exp_lv_point[2] = variavel;
			if (battle_config.weapon_exp_lv_point[2] < battle_config.weapon_exp_lv_point[1]) {
				ShowError("weapon_exp_lv3_point valor especificado '%d' é menor do que weapon_exp_lv2_point.\n", variavel);
			}
		}
		else if (strcmpi(w1, "weapon_exp_lv4_point") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.weapon_exp_lv_point[3] = variavel;
			if (battle_config.weapon_exp_lv_point[3] < battle_config.weapon_exp_lv_point[2]) {
				ShowError("weapon_exp_lv4_point valor especificado '%d' é menor do que weapon_exp_lv3_point.\n", variavel);
			}
		}
		else if (strcmpi(w1, "weapon_exp_lv5_point") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.weapon_exp_lv_point[4] = variavel;
			if (battle_config.weapon_exp_lv_point[4] < battle_config.weapon_exp_lv_point[3]) {
				ShowError("weapon_exp_lv5_point valor especificado '%d' é menor do que weapon_exp_lv4_point.\n", variavel);
			}
		}
		else if (strcmpi(w1, "weapon_exp_lv6_point") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.weapon_exp_lv_point[5] = variavel;
			if (battle_config.weapon_exp_lv_point[5] < battle_config.weapon_exp_lv_point[4]) {
				ShowError("weapon_exp_lv6_point valor especificado '%d' é menor do que weapon_exp_lv5_point.\n", variavel);
			}
		}
		else if (strcmpi(w1, "weapon_exp_lv7_point") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.weapon_exp_lv_point[6] = variavel;
			if (battle_config.weapon_exp_lv_point[6] < battle_config.weapon_exp_lv_point[5]) {
				ShowError("weapon_exp_lv7_point valor especificado '%d' é menor do que weapon_exp_lv6_point.\n", variavel);
			}
		}
		else if (strcmpi(w1, "weapon_exp_lv8_point") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.weapon_exp_lv_point[7] = variavel;
			if (battle_config.weapon_exp_lv_point[7] < battle_config.weapon_exp_lv_point[6]) {
				ShowError("weapon_exp_lv8_point valor especificado '%d' é menor do que weapon_exp_lv7_point.\n", variavel);
			}
		}
		else if (strcmpi(w1, "weapon_exp_lv9_point") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.weapon_exp_lv_point[8] = variavel;
			if (battle_config.weapon_exp_lv_point[8] < battle_config.weapon_exp_lv_point[7]) {
				ShowError("weapon_exp_lv9_point valor especificado '%d' é menor do que weapon_exp_lv8_point.\n", variavel);
			}
		}
		else if (strcmpi(w1, "weapon_exp_lv10_point") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.weapon_exp_lv_point[9] = variavel;
			if (battle_config.weapon_exp_lv_point[9] < battle_config.weapon_exp_lv_point[8]) {
				ShowError("weapon_exp_lv10_point valor especificado '%d' é menor do que weapon_exp_lv9_point.\n", variavel);
			}
		}
		else if (strcmpi(w1, "doton_01_hateffect_area") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doton_01_hateffect_area = variavel;
		}
		else if (strcmpi(w1, "doton_01_hateffectid") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doton_01_hateffectid = variavel;
		}
		else if (strcmpi(w1, "doton_02_hateffect_area") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doton_02_hateffect_area = variavel;
		}
		else if (strcmpi(w1, "doton_02_hateffectid") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.doton_02_hateffectid = variavel;
		}
		else if (strcmpi(w1, "bijuu_01_id") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_01_id = variavel;
		}
		else if (strcmpi(w1, "bijuu_02_id") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_02_id = variavel;
		}
		else if (strcmpi(w1, "bijuu_03_id") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_03_id = variavel;
		}
		else if (strcmpi(w1, "bijuu_04_id") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_04_id = variavel;
		}
		else if (strcmpi(w1, "bijuu_05_id") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_05_id = variavel;
		}
		else if (strcmpi(w1, "bijuu_06_id") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_06_id = variavel;
		}
		else if (strcmpi(w1, "bijuu_07_id") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_07_id = variavel;
		}
		else if (strcmpi(w1, "bijuu_08_id") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_08_id = variavel;
		}
		else if (strcmpi(w1, "bijuu_09_id") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_09_id = variavel;
		}
		else if (strcmpi(w1, "bijuu_01_item") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_01_item = variavel;
		}
		else if (strcmpi(w1, "bijuu_02_item") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_02_item = variavel;
		}
		else if (strcmpi(w1, "bijuu_03_item") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_03_item = variavel;
		}
		else if (strcmpi(w1, "bijuu_04_item") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_04_item = variavel;
		}
		else if (strcmpi(w1, "bijuu_05_item") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_05_item = variavel;
		}
		else if (strcmpi(w1, "bijuu_06_item") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_06_item = variavel;
		}
		else if (strcmpi(w1, "bijuu_07_item") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_07_item = variavel;
		}
		else if (strcmpi(w1, "bijuu_08_item") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_08_item = variavel;
		}
		else if (strcmpi(w1, "bijuu_09_item") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_09_item = variavel;
		}
		else if (strcmpi(w1, "bijuu_01_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_01_chance = variavel;
		}
		else if (strcmpi(w1, "bijuu_02_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_02_chance = variavel;
		}
		else if (strcmpi(w1, "bijuu_03_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_03_chance = variavel;
		}
		else if (strcmpi(w1, "bijuu_04_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_04_chance = variavel;
		}
		else if (strcmpi(w1, "bijuu_05_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_05_chance = variavel;
		}
		else if (strcmpi(w1, "bijuu_06_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_06_chance = variavel;
		}
		else if (strcmpi(w1, "bijuu_07_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_07_chance = variavel;
		}
		else if (strcmpi(w1, "bijuu_08_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_08_chance = variavel;
		}
		else if (strcmpi(w1, "bijuu_09_chance") == 0) {
			sscanf(w2, "%d", &variavel);
			battle_config.bijuu_09_chance = variavel;
		}
		else if (strcmpi(w1, "bijuu_vida") == 0) {
			sscanf(w2, "%lf", &value);
			battle_config.bijuu_vida = value;
		}
	}
	fclose(fp);
...
void battle_defaults(void) {
	...
	battle->skill_config_read = battle_skill_config_read;
	...
	battle->skill_damage_iscaster = battle_skill_damage_iscaster;
	battle->skill_damage_skill = battle_skill_damage_skill;
	battle->skill_damage_map = battle_skill_damage_map;
	battle->skill_damage = battle_skill_damage;
}
