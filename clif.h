...
#define COLOR_VIOLETA    0xBE2CDB // VIOLETA
...
struct clif_interface {
	...
	void (*monster_hp_bar_area) (struct mob_data* md);
	...
	void (*skill_poseffect) (struct block_list *src, struct block_list* dst, uint16 skill_id, int val, int x, int y, int64 tick);
	...
	void(*hat_effect_single2) (struct block_list *bl, uint16 effectId, bool enable);
	void(*hat_effect_single3) (struct block_list* bl, uint16 effectId, enum send_target target, bool enable);
	...
	/* petevolutiom */
	void(*pPetEvolution) (int fd, struct map_session_data *sd, struct s_pet *petinfo);
	...
	/* Aura */
	void(*sendaurastoone) (struct map_session_data *sd, struct map_session_data *dsd);
	void(*sendauras) (struct map_session_data *sd, enum send_target type);
	void(*insight2) (struct block_list *bl, va_list ap);
	void(*getareachar_char) (struct block_list *bl, short flag);
	/* Camera */
	...
	int(*refine_custom) (struct map_session_data *sd, int item_index, int refine_level);
	char *(*bor_pvp_rank) (const struct map_session_data* sd, char* name_pvp);
	/* brAtehna */
