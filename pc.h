...
struct map_session_data {
	...
	struct {
		...
		unsigned int hs_skill_check_double : 1; // Hansip - To check double cast [Kichi]
		unsigned int hs_skill_check_flood : 1; // Hansip - To check flood cast [Kichi]
	} state;
	struct {
	...
	int chat_id;
	int64 idletime;
	int64 keyboard_action_tick;
	int64 keyboard_action_tick_item;
	int64 mouse_action_tick;
	struct {
		int npc_id;
		int64 timeout;
	...
	int64 canskill_tick2;		
	int64 last_skill;			
	int64 tem_tick_skill1;	// Save temporer
	int64 tem_tick_skill2;
	int64 tem_tick_skill3;
	int64 castskill_tick;
	int spam_count;
...
END_ZEROED_BLOCK;
	...
	int hateffecttimer[MAX_EVENTTIMER];
	unsigned short eventcount; // [celest]
	unsigned short hateffectcount;
	...
	int cashPoints, kafraPoints, universePoints;
	int rental_timer;
	...
	struct {
		...
		int itemreqi3cost;
		int itemreqi3chance;
		int blacksmitblessing;
		int blacksmitblessingvalue;
		int refine_chance;
	}evo;
	...
	int nara_marker[MAX_SKILL_NARA_MARKER];
	bool flicker;
...
#define pc_cant_act(sd)       ( (sd)->npc_id || (sd)->state.vending || (sd)->state.buyingstore || (sd)->chat_id || ((sd)->sc.opt1 && (sd)->sc.opt1 != OPT1_BURNING) || (sd)->state.trading || (sd)->state.storage_flag ||  (sd)->state.prevend || (sd)->state.refine_ui == 1)
...
#define pc_isvip(sd) ((sd)->group_id==1 || (sd)->group_id==2)

// Delay para verificação do vip em tempo real. Padrão a cada 1 minuto.
#define DELAY_IN2(x) (x * 60000)
...
#define save_vip(sd,x) \
	sd->group_id = x; pc_set_group(sd, sd->group_id); \
		if(SQL_ERROR == SQL->Query(map->mysql_handle,"UPDATE `%s`.`login` SET `group_id`=%d WHERE `account_id`='%d'", map->mapsdb, x, sd->status.account_id)) \
...
END_ZEROED_BLOCK; /* End */
	...
	bool (*authok) (struct map_session_data *sd, int login_id2, time_t expiration_time, int group_id, const struct mmo_charstatus *st, bool changing_mapservers, const char* mac_address);
	...
	int (*checkequip) (struct map_session_data *sd,int pos, bool checkall);
	...
	// Special Shop System
	int (*paycash) (struct map_session_data *sd, int price, int points, int universe);
	int (*getcash) (struct map_session_data *sd, int cash, int points, int universe);
	...
	int (*equipitem) (struct map_session_data *sd,int n,int req_pos,bool equipswitch);
	...
	int (*weaponexp) (struct map_session_data* sd, int exp);
	int (*weaponexpup) (struct map_session_data* sd, int weaponexp_mob);
