...
struct s_skill_damage {
	...
	int pc,
		...
		boss,
		other;
	int caster, caster2;	/* caster type */
};
...
enum {
	...
	MAPID_BABY_STAR_GLADIATOR = JOBL_BABY | JOBL_2_1 | MAPID_STAR_GLADIATOR,
	...
	MAPID_BABY_SOUL_LINKER = JOBL_BABY | JOBL_2_2 | MAPID_SOUL_LINKER,
	...
	MAPID_SOUL_REAPER      = JOBL_THIRD | JOBL_2_2 | MAPID_SOUL_LINKER,
...
#define DEFAULT_AUTOSAVE_INTERVAL_MAP (5*60*1000)
...
// [pedrodks]
#define map_flag_pvp(m) (map->list[m].flag.pvp)
// [pedrodks]
#define map_flag_fvf(m) (map->list[m].flag.cvc)
...
enum npc_subtype { WARP, SHOP, SCRIPT, CASHSHOP, ITEMSHOP, POINTSHOP, TOMB, MARKETSHOP };
...
enum status_point_types { //we better clean up this enum and change it name [Hemagx]
	...
	SP_ELEDAMAGE, SP_NOTUNEQUIPELMO,
	SP_NOTUNEQUIPASA, SP_NOTUNEQUIPAURA,
	SP_COPYCAT,
...
struct map_data {
	...
	*/
	...
	struct map_flag {
		...
		//pedrodks
		//mapflagranked
		unsigned arena_forfun : 1;
		unsigned arena_extreme : 1;
		unsigned arena_ex_cdesab : 1;
		unsigned arena_gvg : 1;
		unsigned noranked : 1;
	} flag;
	struct point save;
...
END_ZEROED_BLOCK;
	...
	int (*addflooritem) (const struct block_list *bl, struct item *item_data, int amount, int16 m, int16 x, int16 y, int first_charid, int second_charid, int third_charid, int flags, bool showdropeffec);
