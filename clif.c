...
{
	int packet_id = RFIFOW(sd->fd, 0);
	struct socket_data* s = gepard_get_socket_data(sd->fd);
	unsigned int packet_len = (packet_id <= MAX_PACKET_DB) ? packets->db[packet_id] : 0;
	long long diff_time = gepard_get_tick() - s->gepard_info.sync_tick;
	...
	return gepard_process_cs_packet(sd->fd, s, packet_len);
}
// (^~_~^) Gepard Shield End
...
#if PACKETVER >= 20120221
	if (clif->show_monster_hp_bar(bl)) {
		if (battle_config.show_monster_hp_bar == 3) {
			p.maxHP = -1;
			p.HP = -1;
		}
		else {
	...
	switch (bl->type) {
		case BL_PC:
			...
			if (sd->state.size == SZ_BIG) // tiny/big players [Valaris]
					...
					clif->sendauras((TBL_PC*)bl, AREA);
			switch (battle_config.emblem_fac_sys)
			{
				case 1:
					if (sd->status.clan_id != 0 && map->list[sd->bl.m].flag.cvc)
						clif->sc_load(&sd->bl, sd->bl.id, AREA, status->dbs->IconChangeTable[SC_CLAN_INFO], 0, sd->status.clan_id, 0);
				break;
				case 2:
		...
		case BL_MOB:
			...
			if (battle_config.all_hp_monster_map == 1)
				clif->monster_hp_bar_area(md);
			}
			break;
		case BL_NPC:
			{
...
void clif_move(struct unit_data *ud)
{
	...
	nullpo_retv(bl);
	vd = status->get_viewdata(bl);
	if (!vd || vd->class == INVISIBLE_CLASS)
 ...
 *------------------------------------------*/
void clif_addcards(struct EQUIPSLOTINFO *buf, struct item *item)
{
	int i = 0, j;
	nullpo_retv(buf);
	...
	case SP_HP:
		{
			...
			int HP_effects[] = { 150,150,150,150,150,149,149,149,149,149,148,148,148,148,148,147,147,147,147,147,146,146,146,146,146,145,145,145,145,145,144,144,144,144,144,143,143,143,143,143,142,142,142,142,142,141,141,141,141,141,140,140,140,140,140,139,139,139,139,139,138,138,138,138,138,137,137,137,137,137,136,136,136,136,136,135,135,135,135,135,134,134,134,134,134,133,133,133,133,133,132,132,132,132,132,131,131,131,131,131 };
			int HP_effects_red[] = { 223,223,223,223,223,222,222,222,222,222,221,221,221,221,221,220,220,220,220,220,219,219,219,219,219,218,218,218,218,218,217,217,217,217,217,216,216,216,216,216,215,215,215,215,215,214,214,214,214,214,213,213,213,213,213,212,212,212,212,212,211,211,211,211,211,210,210,210,210,210,209,209,209,209,209,208,208,208,208,208,207,207,207,207,207,206,206,206,206,206,205,205,205,205,205,204,204,204,204,204 };

			if (sd->lastEffectID > 0) {
				clif->hat_effect_single2(&sd->bl, sd->lastEffectID, false);
			}

			switch (sd->status.hpcolor)
			{
			case 0:
				clif->hat_effect_single2(&sd->bl, HP_effects_red[max(0, min(percent_hp, 99))], true);
				sd->lastEffectID = HP_effects_red[max(0, min(percent_hp, 99))];
				fd = sd->fd;
				break;
			case 1:
				clif->hat_effect_single2(&sd->bl, HP_effects_red[max(0, min(percent_hp, 99))], false);
				clif->hat_effect_single2(&sd->bl, HP_effects[max(0, min(percent_hp, 99))], true);
	...
	case SP_SP:
		{
			...
			if (sd->lastEffectID2 > 0) {
				clif->hat_effect_single2(&sd->bl, sd->lastEffectID2, false);
			}
			clif->hat_effect_single2(&sd->bl, SP_effects[max(0, min(percent_sp, 99))], true);
...
static void clif_useitemack(struct map_session_data *sd, int index, int amount, bool ok)
{
	...
	if (map->list[sd->bl.m].flag.arena_ex_cdesab && p.itemId >= 19000 && p.itemId <= 19999)
	{
		char buf[256];
		snprintf(buf, sizeof(buf), "[TOP Arena]: Utilizar este tipo de item está proibido nesse mapa!");
	...
	switch (bl->type) {
		case BL_PC:
			...
			if (tsd->state.size == SZ_BIG) // tiny/big players [Valaris]
					...
					clif->sendaurastoone(tsd, sd);
			switch (battle_config.emblem_fac_sys)
			{
				case 1:
					if (tsd->status.clan_id != 0 && map->list[tsd->bl.m].flag.cvc)
						clif->sc_load(&tsd->bl, tsd->bl.id, AREA, status->dbs->IconChangeTable[SC_CLAN_INFO], 0, tsd->status.clan_id, 0);
				break;
				case 2:
...
int clif_damage(struct block_list* src, struct block_list* dst, int sdelay, int ddelay, int64 in_damage, short div, unsigned char type, int64 in_damage2) {
	...
	struct status_change *sc;
	int mobid = status->get_class(dst);
...
void clif_skill_poseffect(struct block_list *src, struct block_list* dst, uint16 skill_id, int val, int x, int y, int64 tick) {
	unsigned char buf[32];
	nullpo_retv(src);
...
static void clif_item_refine_list(struct map_session_data *sd)
{
	...
	p->packetType = 0x221;
	for (i = c = 0; i < sd->status.inventorySize; i++) {
		if (sd->status.inventory[i].nameid > 0 && sd->status.inventory[i].nameid < 25000 && sd->status.inventory[i].identify
...
void clif_party_info(struct party_data* p, struct map_session_data *sd)
{
	...
	for (i = 0, c = 0; i < MAX_PARTY; i++)
	{
		if ((target = p->data[i].sd)) {
			...
			if (target->sc.data[SC_PROTECTWEAPON] && target->sc.data[SC_PROTECTSHIELD] && target->sc.data[SC_PROTECTARMOR] && target->sc.data[SC_PROTECTHELM]) strcat(output, "F");
			else strcat(output, "_");
			...
			memcpy(WBUFP(buf,28+i*46+4), output, NAME_LENGTH);
...
void clif_sendegg(struct map_session_data *sd) {
	...
	fd = sd->fd;
	if (battle_config.pet_no_gvg != 0 && map_flag_gvg2(sd->bl.m)) { //Disable pet hatching in GvG grounds during Guild Wars [Skotlex]
		clif->message(fd, msg_sd(sd, 866)); // "Pets are not allowed in Guild Wars."
		return;
	}
	else if (battle_config.pet_no_pvp != 0 && map_flag_pvp(sd->bl.m)) { // Desabilitar incubação de pet em áreas PvP. [pedrodks]
		//configuração que ativa o uso de bixinhos de estimações em um mapa pvp [pedrodks]
		if (battle_config.active_mappet != 0) {
			if (map->mapname2mapid(map->list[sd->bl.m].name) != pet->mappeti) {
				clif->message(fd, msg_sd(sd, 884)); // "Bicho de estimação não são permitidos em áreas PvP."
				return;
			}
		}
		else {
	...
	else if (battle_config.pet_no_fvf != 0 && map_flag_fvf(sd->bl.m)) { // Desabilitar incubação de pet em áreas FvF. [pedrodks]
		clif->message(fd, msg_sd(sd, 885)); // "Bicho de estimação não são permitidos em Guerras de Facções."
		return;
	}
...
char* clif_bor_pvp_rank(const struct map_session_data* ssd, char* name_pvp)
{
	char rank_e[] = "[E] ";
	char rank_d[] = "[D] ";
	char rank_c[] = "[C] ";
	...
	if (ssd->status.ranked_point <= battle_config.point_rank_d)
		strcat(name_pvp, rank_e);
	else if (ssd->status.ranked_point <= battle_config.point_rank_c)
		strcat(name_pvp, rank_d);
	else if (ssd->status.ranked_point <= battle_config.point_rank_b)
		strcat(name_pvp, rank_c);
	else if (ssd->status.ranked_point <= battle_config.point_rank_a)
		strcat(name_pvp, rank_b);
	else if (ssd->status.ranked_point <= battle_config.point_rank_s)
		strcat(name_pvp, rank_a);
	else if (ssd->status.ranked_point >= battle_config.point_rank_s)
		strcat(name_pvp, rank_s);

	strcat(name_pvp, ssd->status.name);
...
void clif_charnameack (int fd, struct block_list *bl)
{
	...
	switch (bl->type) {
		case BL_PC:
			...
			const struct mmo_charstatus* s = NULL;
				int ps = -1;
			...
			{
				packet.packet_id = reqNameAllType;
				memcpy(packet.guild_name, bg_pos_name[ssd->bg_position], NAME_LENGTH);
				memcpy(packet.position_name, bg_army[bgd->army], NAME_LENGTH);
			}
		...
		case BL_MOB:
			...
			{
				packet.packet_id = reqNameAllType;
				memcpy(packet.guild_name, md->desc, NAME_LENGTH);
				memcpy(packet.position_name, bg_army[bgd->army], NAME_LENGTH);
			}
...
void clif_parse_LoadEndAck(int fd, struct map_session_data *sd) {
	...
	// Spawn pet.
	if (sd->pd != NULL) {
		if (battle_config.pet_no_pvp != 0 && map_flag_pvp(sd->bl.m)) { // Retornar para o ovo. [pedrodks]
			//configuração que ativa o uso de bixinhos de estimações em um mapa pvp [pedrodks]
			if (battle_config.active_mappet != 0) {
				if (map->mapname2mapid(map->list[sd->bl.m].name) != pet->mappeti) {
					clif->message(sd->fd, msg_sd(sd, 882)); // "Bicho de estimação não são permitidos em áreas PvP."
					pet->menu(sd, 3); //Opção 3 retorna pro ovo.
				}
				else {
		...
		else if (battle_config.pet_no_fvf != 0 && map_flag_fvf(sd->bl.m)) { // Retornar para o ovo. [pedrodks]
			clif->message(sd->fd, msg_sd(sd, 883)); // "Bicho de estimação não são permitidos em Guerras de Facções."
			pet->menu(sd, 3); // Opção 3 retorna pro ovo.
		}
		else if (battle_config.pet_no_gvg != 0 && map_flag_gvg2(sd->bl.m)) { // Return the pet to egg. [Skotlex]
	...
	if (sd->state.connect_new != 0) {
		...
		if (map->night_flag != 0 && map->list[sd->bl.m].flag.nightenabled != 0) {
			sd->state.night = 1;
			clif->status_change(&sd->bl, SC_SKE, 1, 0, 0, 0, 0);
		}
	...
	if (sd->state.changemap != 0) { // Restore information that gets lost on map-change.
		...
		if (map->night_flag != 0 && map->list[sd->bl.m].flag.nightenabled != 0) {
			...
			sd->state.night = 0;
			clif->sc_end(&sd->bl, sd->bl.id, SELF, SC_SKE);
		}
...
void clif_parse_GlobalMessage(int fd, struct map_session_data *sd)
{
...

	{
		...
		clif->send(buf, WBUFW(buf, 2), &sd->bl, sd->chat_id ? CHAT_WOS : AREA_CHAT_WOC);
		aFree(buf);
	}
...
void clif_user_count(struct map_session_data* sd, int count) {
	...
	for (md = (TBL_MOB*)mapit->first(iter); mapit->exists(iter); md = (TBL_MOB*)mapit->next(iter)) {
		if (md && md->FakePlayer)
			count++;
	}
...
void clif_parse_ActionRequest_sub(struct map_session_data *sd, int action_type, int target_id, int64 tick)
{
	...
	switch(action_type) {
		...
		case 0x07: // continuous attack
		{
			...
			if( sd->disguise == -1 && map->list[sd->bl.m].flag.zombie  )
				return;
...
void clif_parse_UseItem(int fd, struct map_session_data *sd)
{
	...
	if ( (!sd->npc_id && pc_istrading(sd)) || sd->chat_id)
		return;
...
void clif_parse_EquipItem(int fd,struct map_session_data *sd)
{
	...
	if (sd->inventory_data[index]->type == IT_AMMO)
		pc->equipitem(sd, index, EQP_AMMO, false);
	else
		pc->equipitem(sd, index, p->wearLocation, false);
}
...
void clif_parse_UseSkillToId(int fd, struct map_session_data *sd)
{
	...
	if (battle_config.hansip_protection)
		if (skill->hs_flood_delay_check(sd, skill_id))
			switch (battle_config.macropunit) {

				case 1:
					clif->messagecolor_self(fd, COLOR_CYAN, msg_fd(fd, 3035)); //Calma, vá devagar!!! Está apertando para soltar as habilidades rápido demais!!
					sc_start(NULL, &sd->bl, SC_SILENCE, 100, 0, 0);
				break;

				case 2:
					clif->messagecolor_self(fd, COLOR_CYAN, msg_fd(fd, 3035)); //Calma, vá devagar!!! Está apertando para soltar as habilidades rápido demais!!
					sc_start(NULL, &sd->bl, SC_STONE, 100, 0, 0);
				break;

				case 3:
					clif->messagecolor_self(fd, COLOR_CYAN, msg_fd(fd, 3035)); //Calma, vá devagar!!! Está apertando para soltar as habilidades rápido demais!!
					sc_start(NULL, &sd->bl, SC_FREEZE, 100, 0, 0);
				break;
...
void clif_parse_UseSkillToPosSub(int fd, struct map_session_data *sd, uint16 skill_lv, uint16 skill_id, short x, short y, int skillmoreinfo)
{
	...
	// Hansip Protection [Kichi]
	if (battle_config.hansip_protection)
		skill->hs_flood_delay_check(sd, skill_id);
	//Whether skill fails or not is irrelevant, the char ain't idle. [Skotlex]
...
void clif_parse_MoveToKafra(int fd, struct map_session_data *sd)
{
	...
	if (sd->status.inventory[item_index].equipSwitch) {
		clif->message(sd->fd, msg_sd(sd, 3030)); // "Você não pode colocar itens da aba de troca na Kafra"
		return;
	}
...
void clif_parse_MoveToKafraFromCart(int fd, struct map_session_data *sd)
{
	...
	if (idx < 0 || idx >= sd->status.inventorySize || amount < 1)
		return;
...
void clif_parse_SelectEgg(int fd, struct map_session_data *sd)
{
	...
	printf("ovo selecionado %hu", RFIFOW(fd, 2) - 2);
...
*
**/
static void clif_parse_pet_evolution(int fd, struct map_session_data *sd, struct s_pet *petinfo) __attribute__((nonnull(2)));
/// Request to Evolve the pet (CZ_PET_EVOLUTION) [Dastgir/Hercules]
/// 09fb <Length>.W <EvolvedPetEggID>.W {<index>.W <amount>.W}*items
static void clif_parse_pet_evolution(int fd, struct map_session_data *sd, struct s_pet *petinfo)
{
	...
	for (i = 0; i < VECTOR_LENGTH(pet->db[petIndex].evolve_data); i++) {
		...
		if (ped->petEggId == p->EvolvedPetEggID) {
			...
			for (j = 0; j < VECTOR_LENGTH(ped->items); j++) {
				struct itemlist_entry *list = &VECTOR_INDEX(ped->items, j);
				int n = pc->search_inventory(sd, list->id);
				if (pc->delitem(sd, n, list->amount, 0, DELITEM_NORMAL) == 1) {
					clif->petEvolutionResult(fd, PET_EVOL_NO_MATERIAL);
					return;
				}
			}
			...
			if (pc->delitem(sd, idx, 1, 0, DELITEM_NORMAL) == 1) {
				clif->petEvolutionResult(fd, PET_EVOL_NO_PETEGG);
				return;
			}
			pet_id = pet->search_petDB_index(ped->petEggId, PET_EGG);
			if (pet_id >= 0) {
				uint32_t unique_id = itemdb->pet_unique_id(sd);
				...
				intif->create_pet(
					...
					100, 0, 1, unique_id, pet->db[pet_id].jname);
...
///     ? = nothing
void clif_buyingstore_trade_failed_seller(struct map_session_data* sd, short result, int nameid)
{
#if PACKETVER >= 20100420
	int fd;
...
int clif_vend(struct map_session_data *sd, int skill_lv) {
	...
	if (battle_config.item_universe) {
		WFIFOW(fd, d * 2 + 4) = ITEMID_UNIVERSE;
		d++;
	}
...
void clif_monster_hp_bar(struct mob_data* md, struct map_session_data *sd) {
	...
	//mob
	int effects[] = { 171,171,171,170,170,170,170,170,170,170,169,169,169,169,169,168,168,168,168,168,167,167,167,167,167,166,166,166,166,166,165,165,165,165,165,164,164,164,164,164,163,163,163,163,163,162,162,162,162,162,161,161,161,161,161,160,160,160,160,160,159,159,159,159,159,158,158,158,158,158,157,157,157,157,157,156,156,156,156,156,155,155,155,155,155,154,154,154,154,154,153,153,153,153,153,152,152,152,152,151 };// hateffects_ids para cada %
	int effects2[] = { 192,192,192,191,191,191,190,190,190,190,189,189,189,189,189,188,188,188,188,188,187,187,187,187,187,186,186,186,186,186,185,185,185,185,185,184,184,184,184,184,183,183,183,183,183,182,182,182,182,182,181,181,181,181,181,180,180,180,180,180,179,179,179,179,179,178,178,178,178,178,177,177,177,177,177,176,176,176,176,176,175,175,175,175,175,174,174,174,174,174,173,173,173,173,173,173,173,172,172,172 };// hateffects_ids para cada %
		
	if (md->lastEffectID > 0) {
		clif->hat_effect_single(&md->bl, md->lastEffectID, false);
	}
	...
	switch (md->class_)
	{
		...
		case 1087://orc heroi
		case 1147://maya
		case 1190://senhor dos orcs
		case 1086://besouro ladrão dourado
		case 1115://eddga
		case 1038://osiris
		case 1511://amonra
		case 1159://freoni
		...
		case 1059://abelha rainha
		case 1150://flor do luar
		case 1688://lady tanne
		case 1039://bafomé
		case 1157://farao
		case 1112://drake
		case 1251://cavaleiro da tepestade
		case 1373://senhor dos mortos
		case 1272://senhor das trevas
		case 1885://gorynich
		case 1630://lady branca
		case 1252://Hatii
		case 1779://ktulanux
		case 1708://memoria de thanatos
		case 1623://rsx
		case 1492://samurai incarnado
		case 1418://serpente suprema
		case 1583://tao gunka
		case 1312://general tartaruga
		case 1785://atroce
		case 1734://kiel
		case 1685://vesper
		case 1719://detale
		case 1871://bispo decadente
		case 1768://pesar
		case 1751://valquiria randgris
		case 1832://ifrit
		case 1874://belzebug
		case 1647://algoz eremes
		case 1649://margareta
		case 1650://cecil
		case 1651://katryn
		case 1648://howard
		case 1646://seyren
		case 1905://barricade
			clif->hat_effect_single(&md->bl, effects2[max(0, min(percent_hp, 99))], true);
			md->lastEffectID = effects2[max(0, min(percent_hp, 99))];
		break;
		default:
			clif->hat_effect_single(&md->bl, effects[max(0, min(percent_hp, 99))], true);
...
void clif_parse_CashShopOpen(int fd, struct map_session_data *sd)
{
	...
	//define o botão de cashshop para a ui de refino
	if (battle_config.change_cash_to_refine == 1)
	{
		nullpo_retv(sd);
	...
	else if (battle_config.change_cash_to_refine == 0) {
		...
		const char commandname[] = "optionsaskd";
	...
	for(i = 0; i < limit; i++) {
		...
		if (j < clif->cs.item_count[tab]) {
			...
			if (sd->kafraPoints < kafra_pay) {
				...
				ret = pc->paycash(sd, clif->cs.data[tab][j]->price * qty, kafra_pay, 0);// [Ryuuzaki] //changed Kafrapoints calculation. [Normynator]
				if (ret < 0) {
				...
				for (k = 0; k < qty; k += get_count) {
					if (!pet->create_egg(sd, data->nameid)) {
						...
						if( result != CSBR_SUCCESS )
							pc->getcash(sd, clif->cs.data[tab][j]->price * get_count, 0, 0);
...
void clif_maptypeproperty2(struct block_list *bl, enum send_target t)
{
	...
	p.flag.clan = map->list[bl->m].flag.cvc ? 1 : 0;	// CLAN
...
void clif_package_item_announce(struct map_session_data *sd, int nameid, int containerid) {
...
/* Made Possible Thanks to Yommy! */
void clif_item_drop_announce(struct map_session_data *sd, int nameid, char *monsterName) {
...
static void clif_parse_RouletteOpen(int fd, struct map_session_data* sd)
{
	...
	if (sd->mapindex != mapindex->name2id("konoha")) {
		clif->message(fd, "Permitido utilizar somente em Konoha!");
		return;
	}
	...
	packet_length = sizeof(*p);
	p = (struct packet_roulette_open_ack*)aMalloc(packet_length);
	p->PacketType = 0xa1a;
	...
	if (SQL_ERROR == SQL->Query(map->mysql_handle, "SELECT * FROM `%s` WHERE `account_id`='%d' ", get_database_name(59), sd->status.account_id))
		Sql_ShowDebug(map->mysql_handle);

	while (SQL_SUCCESS == SQL->NextRow(map->mysql_handle)) {

		SQL->GetData(map->mysql_handle, 0, &data, NULL); sd->status.cartas_account_id = atoi(data);
		SQL->GetData(map->mysql_handle, 1, &data, NULL); sd->status.cartas_card[0] = atoi(data);
		SQL->GetData(map->mysql_handle, 2, &data, NULL); sd->status.cartas_quant[0] = atoi(data);
		SQL->GetData(map->mysql_handle, 3, &data, NULL); sd->status.cartas_card[1] = atoi(data);
		SQL->GetData(map->mysql_handle, 4, &data, NULL); sd->status.cartas_quant[1] = atoi(data);
		SQL->GetData(map->mysql_handle, 5, &data, NULL); sd->status.cartas_card[2] = atoi(data);
		SQL->GetData(map->mysql_handle, 6, &data, NULL); sd->status.cartas_quant[2] = atoi(data);
		SQL->GetData(map->mysql_handle, 7, &data, NULL); sd->status.cartas_card[3] = atoi(data);
		SQL->GetData(map->mysql_handle, 8, &data, NULL); sd->status.cartas_quant[3] = atoi(data);
		SQL->GetData(map->mysql_handle, 9, &data, NULL); sd->status.cartas_card[4] = atoi(data);
		SQL->GetData(map->mysql_handle, 10, &data, NULL); sd->status.cartas_quant[4] = atoi(data);
		SQL->GetData(map->mysql_handle, 11, &data, NULL); sd->status.cartas_card[5] = atoi(data);
		SQL->GetData(map->mysql_handle, 12, &data, NULL); sd->status.cartas_quant[5] = atoi(data);
		SQL->GetData(map->mysql_handle, 13, &data, NULL); sd->status.cartas_card[6] = atoi(data);
		SQL->GetData(map->mysql_handle, 14, &data, NULL); sd->status.cartas_quant[6] = atoi(data);
		SQL->GetData(map->mysql_handle, 15, &data, NULL); sd->status.cartas_card[7] = atoi(data);
		SQL->GetData(map->mysql_handle, 16, &data, NULL); sd->status.cartas_quant[7] = atoi(data);
		SQL->GetData(map->mysql_handle, 17, &data, NULL); sd->status.cartas_card[8] = atoi(data);
		SQL->GetData(map->mysql_handle, 18, &data, NULL); sd->status.cartas_quant[8] = atoi(data);
		SQL->GetData(map->mysql_handle, 19, &data, NULL); sd->status.cartas_card[9] = atoi(data);
		SQL->GetData(map->mysql_handle, 20, &data, NULL); sd->status.cartas_quant[9] = atoi(data);
		SQL->GetData(map->mysql_handle, 21, &data, NULL); sd->status.cartas_card[10] = atoi(data);
		SQL->GetData(map->mysql_handle, 22, &data, NULL); sd->status.cartas_quant[10] = atoi(data);
		SQL->GetData(map->mysql_handle, 23, &data, NULL); sd->status.cartas_card[11] = atoi(data);
		SQL->GetData(map->mysql_handle, 24, &data, NULL); sd->status.cartas_quant[11] = atoi(data);
		SQL->GetData(map->mysql_handle, 25, &data, NULL); sd->status.cartas_card[12] = atoi(data);
		SQL->GetData(map->mysql_handle, 26, &data, NULL); sd->status.cartas_quant[12] = atoi(data);
		SQL->GetData(map->mysql_handle, 27, &data, NULL); sd->status.cartas_card[13] = atoi(data);
		SQL->GetData(map->mysql_handle, 28, &data, NULL); sd->status.cartas_quant[13] = atoi(data);
		SQL->GetData(map->mysql_handle, 29, &data, NULL); sd->status.cartas_card[14] = atoi(data);
		SQL->GetData(map->mysql_handle, 30, &data, NULL); sd->status.cartas_quant[14] = atoi(data);
		SQL->GetData(map->mysql_handle, 31, &data, NULL); sd->status.cartas_card[15] = atoi(data);
		SQL->GetData(map->mysql_handle, 32, &data, NULL); sd->status.cartas_quant[15] = atoi(data);
		SQL->GetData(map->mysql_handle, 33, &data, NULL); sd->status.cartas_card[16] = atoi(data);
		SQL->GetData(map->mysql_handle, 34, &data, NULL); sd->status.cartas_quant[16] = atoi(data);
		SQL->GetData(map->mysql_handle, 35, &data, NULL); sd->status.cartas_card[17] = atoi(data);
		SQL->GetData(map->mysql_handle, 36, &data, NULL); sd->status.cartas_quant[17] = atoi(data);
		SQL->GetData(map->mysql_handle, 37, &data, NULL); sd->status.cartas_card[18] = atoi(data);
		SQL->GetData(map->mysql_handle, 38, &data, NULL); sd->status.cartas_quant[18] = atoi(data);
		SQL->GetData(map->mysql_handle, 39, &data, NULL); sd->status.cartas_card[19] = atoi(data);
		SQL->GetData(map->mysql_handle, 40, &data, NULL); sd->status.cartas_quant[19] = atoi(data);
		SQL->GetData(map->mysql_handle, 41, &data, NULL); sd->status.cartas_card[20] = atoi(data);
		SQL->GetData(map->mysql_handle, 42, &data, NULL); sd->status.cartas_quant[20] = atoi(data);
		SQL->GetData(map->mysql_handle, 43, &data, NULL); sd->status.cartas_card[21] = atoi(data);
		SQL->GetData(map->mysql_handle, 44, &data, NULL); sd->status.cartas_quant[21] = atoi(data);
		SQL->GetData(map->mysql_handle, 45, &data, NULL); sd->status.cartas_card[22] = atoi(data);
		SQL->GetData(map->mysql_handle, 46, &data, NULL); sd->status.cartas_quant[22] = atoi(data);
		SQL->GetData(map->mysql_handle, 47, &data, NULL); sd->status.cartas_card[23] = atoi(data);
		SQL->GetData(map->mysql_handle, 48, &data, NULL); sd->status.cartas_quant[23] = atoi(data);
		SQL->GetData(map->mysql_handle, 49, &data, NULL); sd->status.cartas_card[24] = atoi(data);
		SQL->GetData(map->mysql_handle, 50, &data, NULL); sd->status.cartas_quant[24] = atoi(data);
		SQL->GetData(map->mysql_handle, 51, &data, NULL); sd->status.cartas_card[25] = atoi(data);
		SQL->GetData(map->mysql_handle, 52, &data, NULL); sd->status.cartas_quant[25] = atoi(data);
		SQL->GetData(map->mysql_handle, 53, &data, NULL); sd->status.cartas_card[26] = atoi(data);
		SQL->GetData(map->mysql_handle, 54, &data, NULL); sd->status.cartas_quant[26] = atoi(data);
		SQL->GetData(map->mysql_handle, 55, &data, NULL); sd->status.cartas_card[27] = atoi(data);
		SQL->GetData(map->mysql_handle, 56, &data, NULL); sd->status.cartas_quant[27] = atoi(data);
		SQL->GetData(map->mysql_handle, 57, &data, NULL); sd->status.cartas_card[28] = atoi(data);
		SQL->GetData(map->mysql_handle, 58, &data, NULL); sd->status.cartas_quant[28] = atoi(data);
		SQL->GetData(map->mysql_handle, 59, &data, NULL); sd->status.cartas_card[29] = atoi(data);
		SQL->GetData(map->mysql_handle, 60, &data, NULL); sd->status.cartas_quant[29] = atoi(data);
		SQL->GetData(map->mysql_handle, 61, &data, NULL); sd->status.cartas_card[30] = atoi(data);
		SQL->GetData(map->mysql_handle, 62, &data, NULL); sd->status.cartas_quant[30] = atoi(data);
		SQL->GetData(map->mysql_handle, 63, &data, NULL); sd->status.cartas_card[31] = atoi(data);
		SQL->GetData(map->mysql_handle, 64, &data, NULL); sd->status.cartas_quant[31] = atoi(data);
		SQL->GetData(map->mysql_handle, 65, &data, NULL); sd->status.cartas_card[32] = atoi(data);
		SQL->GetData(map->mysql_handle, 66, &data, NULL); sd->status.cartas_quant[32] = atoi(data);
		SQL->GetData(map->mysql_handle, 67, &data, NULL); sd->status.cartas_card[33] = atoi(data);
		SQL->GetData(map->mysql_handle, 68, &data, NULL); sd->status.cartas_quant[33] = atoi(data);
		SQL->GetData(map->mysql_handle, 69, &data, NULL); sd->status.cartas_card[34] = atoi(data);
		SQL->GetData(map->mysql_handle, 70, &data, NULL); sd->status.cartas_quant[34] = atoi(data);
		SQL->GetData(map->mysql_handle, 71, &data, NULL); sd->status.cartas_card[35] = atoi(data);
		SQL->GetData(map->mysql_handle, 72, &data, NULL); sd->status.cartas_quant[35] = atoi(data);
		SQL->GetData(map->mysql_handle, 73, &data, NULL); sd->status.cartas_card[36] = atoi(data);
		SQL->GetData(map->mysql_handle, 74, &data, NULL); sd->status.cartas_quant[36] = atoi(data);
		SQL->GetData(map->mysql_handle, 75, &data, NULL); sd->status.cartas_card[37] = atoi(data);
		SQL->GetData(map->mysql_handle, 76, &data, NULL); sd->status.cartas_quant[37] = atoi(data);
		SQL->GetData(map->mysql_handle, 77, &data, NULL); sd->status.cartas_card[38] = atoi(data);
		SQL->GetData(map->mysql_handle, 78, &data, NULL); sd->status.cartas_quant[38] = atoi(data);
		SQL->GetData(map->mysql_handle, 79, &data, NULL); sd->status.cartas_card[39] = atoi(data);
		SQL->GetData(map->mysql_handle, 80, &data, NULL); sd->status.cartas_quant[39] = atoi(data);
		SQL->GetData(map->mysql_handle, 81, &data, NULL); sd->status.cartas_card[40] = atoi(data);
		SQL->GetData(map->mysql_handle, 82, &data, NULL); sd->status.cartas_quant[40] = atoi(data);
		SQL->GetData(map->mysql_handle, 83, &data, NULL); sd->status.cartas_card[41] = atoi(data);
		SQL->GetData(map->mysql_handle, 84, &data, NULL); sd->status.cartas_quant[41] = atoi(data);
	}
	SQL->FreeResult(map->mysql_handle);

	for (i; i < 42; i++) {
		if (sd->status.cartas_quant[i] > 0 && i < 11)
			k = k + sd->status.cartas_quant[i];
		else if (sd->status.cartas_quant[i] > 0 && i < 36)
			j = j + sd->status.cartas_quant[i];
		else if (sd->status.cartas_quant[i] > 0 && i < 42)
			l = l + sd->status.cartas_quant[i];
	}
	p->BronzePoint = k;
	p->GoldPoint = l;
	p->SilverPoint = j;

	clif->pRouletteInfo(fd, sd);
	clif->send(p, packet_length, &sd->bl, SELF);
	aFree(p);
#endif
}
...
static void clif_parse_RouletteInfo(int fd, struct map_session_data* sd)
{
	...
	packet_length = sizeof(*p);
	p = (struct packet_roulette_info_ack *)aMalloc(packet_length);
	...
	for (i = 0; i < MAX_ROULETTE_LEVEL; i++) {
		for (j = 0; j < MAX_ROULETTE_COLUMNS - i; j++) {
			...
			if (rd.nameid[i][j] == sd->status.cartas_card[count])
			{
				p->ItemInfo[count].ItemId = sd->status.cartas_card[count];
				if (sd->status.cartas_quant[count] != 0)
					p->ItemInfo[count].Count = sd->status.cartas_quant[count];
				else
					p->ItemInfo[count].Count = -1;
			}
			else {
...
static void clif_parse_RouletteClose(int fd, struct map_session_data* sd)
{
	...
	struct packet_roulette_info_ack r;
	memset(&r, 0, sizeof(r));
	struct packet_roulette_open_ac* h;
	memset(&h, 0, sizeof(h));
...
//	if (sd->state.trading || pc_isdead(sd) || pc_isvending(sd))
//		return;
	npc->event(sd, "albumBOR::OnBotao01", 0);
	...
	if (result == GENERATE_ROULETTE_SUCCESS) {
		...
		if (sd->roulette.prizeIdx == 0) {
			...
			pc->additem(sd, &it, rd.qty[stage][0]); //TODO maybe a new log type for roulette items?
...
//	if (sd->state.trading || pc_isdead(sd) || pc_isvending(sd))
//		return;
	npc->event(sd, "albumBOR::OnBotao02", 0);
	...
	if (sd->roulette.claimPrize) {
		...
		switch (pc->additem(sd, &it, rd.qty[sd->roulette.prizeStage][sd->roulette.prizeIdx])) {
		case 0:
...
void clif_rodex_add_item_result(struct map_session_data *sd, int16 idx, int16 amount, enum rodex_add_item result)
{
...
void clif_rodex_request_zeny(struct map_session_data *sd, int8 opentype, int64 mail_id, enum rodex_get_zeny result)
{
...
void clif_rodex_request_items(struct map_session_data *sd, int8 opentype, int64 mail_id, enum rodex_get_items result)
{
#if PACKETVER >= 20140409
...
void clif_hat_effect_single3(struct block_list* bl, uint16 effectId, enum send_target target, bool enable) {
#if PACKETVER >= 20150422
	unsigned char buf[13];
...
static void clif_open_ui(struct map_session_data *sd, enum cz_ui_types uiType)
{
	...
	case CZ_ATTENDANCE_UI:
	{
		...
		if (battle_config.feature_enable_attendance_system != 1)
			return;
		if(battle_config.prem_diar_vip == 1) {
			if (sd->group_id == 0) {
				clif->messagecolor_self(sd->fd, COLOR_RED, "Função habilitada apenas para VIP's."); // O portão foi aberto.
				return;
			}
		}
#if PACKETVER_RE_NUM >= 20180307 || PACKETVER_MAIN_NUM >= 20180404 || PACKETVER_ZERO_NUM >= 20180411
...
void clif_parse_attendance_reward_request(int fd, struct map_session_data *sd)
{
	...
	msg.receiver_accountid = sd->status.account_id;
...
void clif_stylist_send_rodexitem(struct map_session_data *sd, int itemid)
{
	struct rodex_message msg = { 0 };
...
void clif_parse_equipswitch_request(int fd, struct map_session_data* sd) {
	...
	sd->equipswitch_tick = tick + skill->get_cooldown(skill_id, skill_lv);
	...
	if (pc_issit(sd)) {
		clif->message(sd->fd, msg_sd(sd, 3032)); // "A troca de equipamento não funciona com o personagem sentado"
		return;
	}
...
void clif_parse_equipswitch_request_single(int fd, struct map_session_data* sd) {
	...
	if (sd->status.inventory[index].equipSwitch) {
		...
		pc->equipitem(sd, index, pc->equippoint(sd, index), true);
	}
#endif
}
...
static void clif_AddItemRefineryUIAck(struct map_session_data *sd, int item_index, struct s_refine_requirement *req)
{
	...
	nullpo_retv(sd);
	nullpo_retv(req);
	Assert_retv(item_index >= 0 && item_index < MAX_INVENTORY);

	if (battle_config.enable_refinery_ui == 0)
		return;

	if (battle_config.block_weapons_refine == 1 && sd->status.inventory[item_index].nameid < battle_config.id_item_refine_apartir) {
		clif->message(sd->fd, msg_sd(sd, 3044));//Não pode prosseguir. Tente com outro equipamento.
		return;
	}

	if (sd->status.inventory[item_index].card[0] > 0
		|| sd->status.inventory[item_index].card[1] > 0
		|| sd->status.inventory[item_index].card[2] > 0
		|| sd->status.inventory[item_index].card[3] > 0
		) {
		clif->message(sd->fd, msg_sd(sd, 3041));//Desequipe a carta do seu equipamento antes de evoluir.
		return;
	}
	...
	switch (clif->refine_custom(sd, sd->status.inventory[item_index].nameid, refine_level))
	{
		case -1:
			return;
		break;
		case 1:
			for (int i = 0; i < 4; ++i) {
				if (i == 0) {
					p->req[i].chance = sd->evo.itemreqi0chance;
					p->req[i].itemId = req->req[i].nameid = sd->evo.itemreqi0;
					p->req[i].zeny = req->req[i].cost = sd->evo.itemreqi0cost;
					p->blacksmithBlessing = req->blacksmith_blessing = sd->evo.blacksmitblessing;
				}
				else if (i == 1) {
					p->req[i].chance = sd->evo.itemreqi1chance;
					p->req[i].itemId = req->req[i].nameid = sd->evo.itemreqi1;
					p->req[i].zeny = req->req[i].cost = sd->evo.itemreqi1cost;
					p->blacksmithBlessing = req->blacksmith_blessing = sd->evo.blacksmitblessing;
				}
				else if (i == 2) {
					p->req[i].chance = sd->evo.itemreqi2chance;
					p->req[i].itemId = req->req[i].nameid = sd->evo.itemreqi2;
					p->req[i].zeny = req->req[i].cost = sd->evo.itemreqi2cost;
					p->blacksmithBlessing = req->blacksmith_blessing = sd->evo.blacksmitblessing;
				}
				else if (i == 3) {
					p->req[i].chance = sd->evo.itemreqi3chance;
					p->req[i].itemId = req->req[i].nameid = sd->evo.itemreqi3;
					p->req[i].zeny = req->req[i].cost = sd->evo.itemreqi3cost;
					p->blacksmithBlessing = req->blacksmith_blessing = sd->evo.blacksmitblessing;
				}
			}
		break;
...
//[pedrodks]
//Função retorna se o item é uma asa ou arma para fazer a seleção de itens de evolução
//Também salva os itens map_session_data
static int clif_refine_custom(struct map_session_data *sd, int item_index, int refine_level)
{
	clif->message(sd->fd, "Sistema Desativado");
	return 0;
}
...
void clif_bc_ready(void) {
	...
	if (is_gepard_active == true)
	{
		battle_config.packet_obfuscation = 0;
	}
	// (^~_~^) Gepard Shield End
...
void clif_defaults(void) {
	...
	clif->monster_hp_bar_area = clif_monster_hp_bar_area;
	...
	clif->hat_effect_single2 = clif_hat_effect_single2;
	clif->hat_effect_single3 = clif_hat_effect_single3;
	...
	// -- Aura
	clif->sendaurastoone = clif_sendaurastoone;
	clif->sendauras = clif_sendauras;
	clif->insight2 = clif_insight2;
	clif->getareachar_char = clif_getareachar_char;
	// -- Refino
	...
	clif->refine_custom = clif_refine_custom;
	clif->bor_pvp_rank = clif_bor_pvp_rank;
}