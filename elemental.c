...
int read_elementaldb(void) {
	struct s_elemental_db *db;
	struct config_t elemental_config;
	struct config_setting_t * ele_db = NULL;
	int ele_count=0;
	
	int count = 0, ele;

	const char *config_filename = "db/borutodb/Elemental.conf";
	memset(elemental->db,0,sizeof(elemental->db));
	
	if (!libconfig->load_file(&elemental_config, config_filename))
		return -1;
	...
	if(ele_db != NULL){
		...
		for(count = 0; count < ele_count && count < MAX_ELEMENTAL_CLASS; count++){
			...
			struct config_setting_t * elemental_ = libconfig->setting_get_elem(ele_db, count);
