...
int pc_makesavestatus(struct map_session_data *sd)
{
	...
	if( ( map->list[sd->bl.m].flag.nosave && sd->state.autotrade != 2 ) || map->list[sd->bl.m].instance_id >= 0 || map->getcell( sd->bl.m, &sd->bl, sd->bl.x, sd->bl.y, CELL_CHKPVP ) ){
...
int pc_setequipindex(struct map_session_data *sd)
{
	...
	for (i = 0; i < sd->status.inventorySize; i++) {
		...
		if (sd->status.inventory[i].equipSwitch) {
			for (j=0;j<EQI_MAX;j++) {
				if (sd->status.inventory[i].equipSwitch & pc->equip_pos[j]) {
...
int pc_isequip(struct map_session_data *sd,int n)
{
	...
	if ( map->list[sd->bl.m].flag.noweapon )
 ...
 *------------------------------------------*/
bool pc_authok(struct map_session_data *sd, int login_id2, time_t expiration_time, int group_id, const struct mmo_charstatus *st, bool changing_mapservers, const char* mac_address)
{
	int i;
...
//		sd->vip_timer = timer->add(timer->gettick()+1000, pc->check_time_vip, sd->bl.id, 0);
		...
		else if(sd->group_id==1)
			sd->vip_timer2 = timer->add(timer->gettick()+DELAY_IN2(1), pc->check_time_vip2, sd->bl.id, 0);
	}
...
int pc_reg_received(struct map_session_data *sd)
{
	...
	// @aura
	sd->status.aura = pc_readglobalreg(sd,script->add_variable("USERAURA"));

	// @hpcolor
	sd->status.hpcolor = pc_readglobalreg(sd, script->add_variable("HPCOLOR"));
	...
	// universe
	sd->universePoints = pc_readaccountreg(sd,script->add_variable("#UNIVERSEPOINTS"));
	...
	if( pc_readaccountreg(sd,script->add_variable("#BLOCKPASS")) > 0 )
...
int pc_bonus_autospell(struct s_autospell *spell, int max, short id, short lv, short rate, short flag, int card_id)
{
	int i;
...
int pc_bonus_autospell_onskill(struct s_autospell *spell, int max, short src_skill, short id, short lv, short rate, int card_id)
{
	int i;
...
int pc_bonus(struct map_session_data *sd,int type,int val) {
	...
	switch(type){
		...
		case SP_FLEE2:
			...
			break;
		case SP_NOTUNEQUIPELMO:
			bst->unequip_elmo += val;
			break;
		case SP_NOTUNEQUIPASA:
			bst->unequip_asa += val;
			break;
		case SP_NOTUNEQUIPAURA:
			bst->unequip_aura += val;
			break;
		case SP_CRITICAL:
			if(sd->state.lr_flag != 2) {
...
int pc_bonus2(struct map_session_data *sd,int type,int type2,int val)
{
	...
	switch(type){
		...
		case SP_ELEDAMAGE:
			if ((type2 >= ELE_MAX && type2 != ELE_ALL) || (type2 < ELE_NEUTRAL)) {
				ShowError("pc_bonus2: SP_ELEDAMAGE: Elemento Invalido %d\n", type2);
				break;
			}
			if (type2 == ELE_ALL) {
				for (i = ELE_NEUTRAL; i < ELE_MAX; i++)
 ...
 * Cash Shop
 *------------------------------------------*/
int pc_paycash(struct map_session_data *sd, int price, int points, int universe)
{
	int cash, type;
	int mempoints;
	nullpo_retr(-1, sd);
	...
	if (universe){
		pc_setaccountreg(sd, script->add_variable("#UNIVERSEPOINTS"), sd->universePoints - cash);
	}
	else
	{
	...
	if( battle_config.cashshop_show_points )
	{
		char output[128];
		if( universe )
			sprintf(output, msg_sd(sd,1602), cash, sd->universePoints);
		else 
...
int pc_getcash(struct map_session_data *sd, int cash, int points, int universe)
{
	char output[128];
	int type;
	nullpo_retr(-1,sd);
	...
	if( cash > 0 )
	{
		...
		{
			if( universe )
				sprintf(output, msg_sd(sd,1603), cash, sd->universePoints);
			else 
 ...
 * 7 = stack limitation
 *------------------------------------------*/
static int pc_additem(struct map_session_data *sd, struct item *item_data, int amount)
{
	struct item_data *data;
	int i;
	unsigned int w;
	...
	if( ( !itemdb->isstackable2(data) || data->flag.force_serial || data->type == IT_CASH) && !item_data->unique_id ) {
		if (!data->type == IT_PETEGG) {
			sd->status.inventory[i].unique_id = itemdb->unique_id(sd);
			item_data->unique_id = sd->status.inventory[i].unique_id;
		} else{
			sd->status.inventory[i].unique_id = itemdb->pet_unique_id(sd);
			item_data->unique_id = sd->status.inventory[i].unique_id;
		}
	}
	...
	if(data->flag.autoequip)
		pc->equipitem(sd, i, data->equip,false);
 ...
 *------------------------------------------*/
static int pc_delitem(struct map_session_data *sd,int n,int amount,int type, short reason)
{
	nullpo_retr(1, sd);
...
int pc_isUseitem(struct map_session_data *sd,int n)
{
	...
	switch( nameid ) { // TODO: Is there no better way to handle this, other than hardcoding item IDs?
		...
		case ITEMID_SIEGE_TELEPORT_SCROLL:
			...
			if (nameid != ITEMID_WING_OF_FLY && nameid != ITEMID_GIANT_FLY_WING && map->list[sd->bl.m].flag.noreturn || map->getcell(sd->bl.m, &sd->bl, sd->bl.x, sd->bl.y, CELL_CHKPVP)) {
#if PACKETVER >= 20080311
				clif->skill_mapinfomessage(sd, 0);
#else
				clif->messagecolor_self(sd->fd, COLOR_RED, msg_sd(sd, 49));
...
int pc_useitem (struct map_session_data *sd, int n)
{
		...
		if( i < MAX_ITEMDELAYS ) {
			if( sd->item_delay[i].nameid ) {// found
				if( DIFF_TICK(sd->item_delay[i].tick, tick) > 0 ) {
					...
					clif->msgtable_num(sd, MSG_ITEM_REUSE_LIMIT_SECOND, e_tick + 1); // [%d] seconds left until you can use
	...
	if (sd->status.inventory[n].card[0] == CARD0_CREATE && pc->fame_rank(MakeDWord(sd->status.inventory[n].card[2], sd->status.inventory[n].card[3]), RANKTYPE_ALCHEMIST) > 0) {
...
int pc_cart_additem(struct map_session_data *sd,struct item *item_data,int amount)
{
	...
	sd->status.cart[i].equipSwitch = 0;
...
int pc_putitemtocart(struct map_session_data *sd,int idx,int amount)
{
	...
	if (item_data->equipSwitch) {
		clif->message(sd->fd, msg_sd(sd, 3031)); // "Você não pode colocar itens da aba de troca no Carrinho";
		return 0;
	}
...
const char *job_name(int class)
{
	...
	case JOB_BABY_REBELLION:
		return msg_txt(753 - JOB_BABY_KAGEROU+class);
	...
	case JOB_BABY_SOUL_REAPER:
		return msg_txt(782 - JOB_STAR_EMPEROR+class);
...
bool pc_gainexp(struct map_session_data *sd, struct block_list *src, uint64 base_exp, uint64 job_exp, bool is_quest)
{
	...
	if(sd->state.showexp) {
		char output[256];
		sprintf(output,
			"Experiência Base:%"PRIu64" (%.2f%%) Classe:%"PRIu64" (%.2f%%)", base_exp, nextbp * (float)100, job_exp, nextjp * (float)100);
		clif_disp_onlyself(sd,output);
	}
...
//Sistema de WeaponExp
int pc_weaponexp(struct map_session_data* sd, int exp)
{
	int i = 0, level = 0;

	for (i; i < sizeof(battle_config.weapon_exp_lv_point); i++)
	{
		if (exp < battle_config.weapon_exp_lv_point[i])
		{
			level = i;
			break;
		}
...
int pc_weaponexpup(struct map_session_data* sd, int weaponexp_mob)
{
	char buf[256];
	int level_atual = 0;
	int level_apos = 0;
	...
	if (weaponexp_mob > 0)
	{
		level_atual = pc->weaponexp(sd, sd->status.weaponexp);

		//pedrodks weaponexp
		sd->status.weaponexp += weaponexp_mob;

		level_apos = pc->weaponexp(sd, sd->status.weaponexp);

		if (level_apos <= level_atual)
		{
			snprintf(buf, sizeof(buf), "[WeaponExp]: %d de WeaponExp Obtidos.", weaponexp_mob);
			clif->messagecolor_self(sd->fd, COLOR_CYAN, buf);
		}
		else {
			clif->specialeffect_single(&sd->bl, 12, sd->fd);
			clif->specialeffect_single(&sd->bl, 376, sd->fd);
			clif->soundeffect(sd, &sd->bl, "narutolevelup.wav", BL_PC);
			snprintf(buf, sizeof(buf), "[WeaponExp]: %d de WeaponExp Obtidos, Seu level subiu de Lv.%d para o Lv.%d.", weaponexp_mob, level_atual, level_apos);
			clif->messagecolor_self(sd->fd, COLOR_YELLOW, buf);
		}
		return 0;
	}
...
//Sistema Rankeada
int pc_rankeada(struct map_session_data* matou, struct map_session_data* morreu, bool estilo)
{
	char buf[256];

	if (map->list[matou->bl.m].flag.noranked || map->list[morreu->bl.m].flag.noranked) {
		return 1;
	}

	//atualização de quem matou
	if (estilo == true)
	{
		if (matou->status.ranked_point <= battle_config.point_rank_d) {
			if (morreu->status.ranked_point <= battle_config.point_rank_d) {
				matou->status.ranked_point += battle_config.rank_e_mata_e;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_e_mata_e, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_c) {
				matou->status.ranked_point += battle_config.rank_e_mata_d;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_e_mata_d, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_b) {
				matou->status.ranked_point += battle_config.rank_e_mata_c;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_e_mata_c, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_a) {
				matou->status.ranked_point += battle_config.rank_e_mata_b;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_e_mata_b, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_s) {
				matou->status.ranked_point += battle_config.rank_e_mata_a;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_e_mata_a, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point > battle_config.point_rank_s) {
				matou->status.ranked_point += battle_config.rank_e_mata_s;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_e_mata_s, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
		}
		else if (matou->status.ranked_point < battle_config.point_rank_c) {
			if (morreu->status.ranked_point <= battle_config.point_rank_d) {
				matou->status.ranked_point += battle_config.rank_d_mata_e;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_d_mata_e, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_c) {
				matou->status.ranked_point += battle_config.rank_d_mata_d;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_d_mata_d, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_b) {
				matou->status.ranked_point += battle_config.rank_d_mata_c;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_d_mata_c, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_a) {
				matou->status.ranked_point += battle_config.rank_d_mata_b;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_d_mata_b, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_s) {
				matou->status.ranked_point += battle_config.rank_d_mata_a;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_d_mata_a, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point > battle_config.point_rank_s) {
				matou->status.ranked_point += battle_config.rank_d_mata_s;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_d_mata_s, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
		}
		else if (matou->status.ranked_point < battle_config.point_rank_b) {
			if (morreu->status.ranked_point <= battle_config.point_rank_d) {
				matou->status.ranked_point += battle_config.rank_c_mata_e;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_c_mata_e, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_c) {
				matou->status.ranked_point += battle_config.rank_c_mata_d;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_c_mata_d, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_b) {
				matou->status.ranked_point += battle_config.rank_c_mata_c;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_c_mata_c, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_a) {
				matou->status.ranked_point += battle_config.rank_c_mata_b;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_c_mata_b, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_s) {
				matou->status.ranked_point += battle_config.rank_c_mata_a;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_c_mata_a, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point > battle_config.point_rank_s) {
				matou->status.ranked_point += battle_config.rank_c_mata_s;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_c_mata_s, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
		}
		else if (matou->status.ranked_point < battle_config.point_rank_a) {
			if (morreu->status.ranked_point <= battle_config.point_rank_d) {
				matou->status.ranked_point += battle_config.rank_b_mata_e;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_b_mata_e, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_c) {
				matou->status.ranked_point += battle_config.rank_b_mata_d;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_b_mata_d, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_b) {
				matou->status.ranked_point += battle_config.rank_b_mata_c;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_b_mata_c, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_a) {
				matou->status.ranked_point += battle_config.rank_b_mata_b;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_b_mata_b, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_s) {
				matou->status.ranked_point += battle_config.rank_b_mata_a;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_b_mata_a, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point > battle_config.point_rank_s) {
				matou->status.ranked_point += battle_config.rank_b_mata_s;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_b_mata_s, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
		}
		else if (matou->status.ranked_point < battle_config.point_rank_s) {
			if (morreu->status.ranked_point <= battle_config.point_rank_d) {
				matou->status.ranked_point += battle_config.rank_a_mata_e;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_a_mata_e, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_c) {
				matou->status.ranked_point += battle_config.rank_a_mata_d;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_a_mata_d, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_b) {
				matou->status.ranked_point += battle_config.rank_a_mata_c;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_a_mata_c, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_a) {
				matou->status.ranked_point += battle_config.rank_a_mata_b;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_a_mata_b, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_s) {
				matou->status.ranked_point += battle_config.rank_a_mata_a;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_a_mata_a, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point > battle_config.point_rank_s) {
				matou->status.ranked_point += battle_config.rank_a_mata_s;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_a_mata_s, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
		}
		else if (matou->status.ranked_point > battle_config.point_rank_s) {
			if (morreu->status.ranked_point <= battle_config.point_rank_d) {
				matou->status.ranked_point += battle_config.rank_s_mata_e;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_s_mata_e, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_c) {
				matou->status.ranked_point += battle_config.rank_s_mata_d;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_s_mata_d, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_b) {
				matou->status.ranked_point += battle_config.rank_s_mata_c;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_s_mata_c, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_a) {
				matou->status.ranked_point += battle_config.rank_s_mata_b;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_s_mata_b, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point <= battle_config.point_rank_s) {
				matou->status.ranked_point += battle_config.rank_s_mata_a;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_s_mata_a, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
			else if (morreu->status.ranked_point >= battle_config.point_rank_s) {
				matou->status.ranked_point += battle_config.rank_s_mata_s;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Ganhou [%d] pontos por matar [%s]", battle_config.rank_s_mata_s, morreu->status.name);
				clif->messagecolor_self(matou->fd, COLOR_ORANGE, buf);
			}
		}

		//atualiza o nome de quem matou
		clif->charnameack(0, &matou->bl);
	}
	//atualização de quem morreu
	else {
		if (morreu->status.ranked_point <= 0) {
			morreu->status.ranked_point = 0;
			snprintf(buf, sizeof(buf), "[WoR Ranked]: Lute e chegue ao topo!");
			clif->messagecolor_self(morreu->fd, COLOR_CYAN, buf);
			return 1;
		}
		if (morreu->status.ranked_point <= battle_config.point_rank_d) {
			if (matou->status.ranked_point <= battle_config.point_rank_d) {
				morreu->status.ranked_point -= battle_config.rank_e_morre_e;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_e_morre_e, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_c) {
				morreu->status.ranked_point -= battle_config.rank_e_morre_d;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_e_morre_d, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_b) {
				morreu->status.ranked_point -= battle_config.rank_e_morre_c;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_e_morre_c, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_a) {
				morreu->status.ranked_point -= battle_config.rank_e_morre_b;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_e_morre_b, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_s) {
				morreu->status.ranked_point -= battle_config.rank_e_morre_a;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_e_morre_a, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (morreu->status.ranked_point > battle_config.point_rank_s) {
				morreu->status.ranked_point -= battle_config.rank_e_morre_s;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_e_morre_s, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
		}
		else if (morreu->status.ranked_point < battle_config.point_rank_c) {
			if (matou->status.ranked_point <= battle_config.point_rank_d) {
				morreu->status.ranked_point -= battle_config.rank_d_morre_e;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_d_morre_e, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_c) {
				morreu->status.ranked_point -= battle_config.rank_d_morre_d;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_d_morre_d, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_b) {
				morreu->status.ranked_point -= battle_config.rank_d_morre_c;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_d_morre_c, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_a) {
				morreu->status.ranked_point -= battle_config.rank_d_morre_b;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_d_morre_b, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_s) {
				morreu->status.ranked_point -= battle_config.rank_d_morre_a;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_d_morre_a, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (morreu->status.ranked_point > battle_config.point_rank_s) {
				morreu->status.ranked_point -= battle_config.rank_d_morre_s;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_d_morre_s, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
		}
		else if (morreu->status.ranked_point < battle_config.point_rank_b) {
			if (matou->status.ranked_point <= battle_config.point_rank_d) {
				morreu->status.ranked_point -= battle_config.rank_c_morre_e;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_c_morre_e, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_c) {
				morreu->status.ranked_point -= battle_config.rank_c_morre_d;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_c_morre_d, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_b) {
				morreu->status.ranked_point -= battle_config.rank_c_morre_c;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_c_morre_c, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_a) {
				morreu->status.ranked_point -= battle_config.rank_c_morre_b;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_c_morre_b, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_s) {
				morreu->status.ranked_point -= battle_config.rank_c_morre_a;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_c_morre_a, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (morreu->status.ranked_point > battle_config.point_rank_s) {
				morreu->status.ranked_point -= battle_config.rank_c_morre_s;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_c_morre_s, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
		}
		else if (morreu->status.ranked_point < battle_config.point_rank_a) {
			if (matou->status.ranked_point <= battle_config.point_rank_d) {
				morreu->status.ranked_point -= battle_config.rank_b_morre_e;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_b_morre_e, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_c) {
				morreu->status.ranked_point -= battle_config.rank_b_morre_d;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_b_morre_d, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_b) {
				morreu->status.ranked_point -= battle_config.rank_b_morre_c;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_b_morre_c, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_a) {
				morreu->status.ranked_point -= battle_config.rank_b_morre_b;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_b_morre_b, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_s) {
				morreu->status.ranked_point -= battle_config.rank_b_morre_a;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_b_morre_a, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (morreu->status.ranked_point > battle_config.point_rank_s) {
				morreu->status.ranked_point -= battle_config.rank_b_morre_s;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_b_morre_s, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
		}
		else if (morreu->status.ranked_point < battle_config.point_rank_s) {
			if (matou->status.ranked_point <= battle_config.point_rank_d) {
				morreu->status.ranked_point -= battle_config.rank_a_morre_e;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_a_morre_e, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_c) {
				morreu->status.ranked_point -= battle_config.rank_a_morre_d;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_a_morre_d, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_b) {
				morreu->status.ranked_point -= battle_config.rank_a_morre_c;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_a_morre_c, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_a) {
				morreu->status.ranked_point -= battle_config.rank_a_morre_b;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_a_morre_b, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_s) {
				morreu->status.ranked_point -= battle_config.rank_a_morre_a;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_a_morre_a, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (morreu->status.ranked_point > battle_config.point_rank_s) {
				morreu->status.ranked_point -= battle_config.rank_a_morre_s;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_a_morre_s, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
		}
		else if (morreu->status.ranked_point >= battle_config.point_rank_s) {
			if (matou->status.ranked_point <= battle_config.point_rank_d) {
				morreu->status.ranked_point -= battle_config.rank_s_morre_e;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_s_morre_e, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_c) {
				morreu->status.ranked_point -= battle_config.rank_s_morre_d;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_s_morre_d, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_b) {
				morreu->status.ranked_point -= battle_config.rank_s_morre_c;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_s_morre_c, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_a) {
				morreu->status.ranked_point -= battle_config.rank_s_morre_b;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_s_morre_b, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (matou->status.ranked_point <= battle_config.point_rank_s) {
				morreu->status.ranked_point -= battle_config.rank_s_morre_a;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_s_morre_a, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
			else if (morreu->status.ranked_point > battle_config.point_rank_s) {
				morreu->status.ranked_point -= battle_config.rank_s_morre_s;
				snprintf(buf, sizeof(buf), "[WoR Ranked]: Perdeu [%d] pontos por morrer para [%s]", battle_config.rank_s_morre_s, matou->status.name);
				clif->messagecolor_self(morreu->fd, COLOR_RED, buf);
			}
		}

		//atualiza o nome de quem morreu
		clif->charnameack(0, &morreu->bl);
	}
...
int pc_dead(struct map_session_data *sd,struct block_list *src) {
	...
	//calculo de pontos da rankeada [pedrodks/luminus]
	if (battle_config.ranked_system_ativado == 1) {
		if (src != NULL && src->type == BL_PC) {
			struct map_session_data* ssd = BL_UCAST(BL_PC, src);
			pc->pc_rankeada(ssd, sd, false);
		}
	}
	...
	if (battle_config.death_penalty_type
	   ...
	   && !sd->sc.data[SC_BABY] && !sd->sc.data[SC_CASH_DEATHPENALTY]
		...
		if (battle_config.zeny_penalty > 0 && !map->list[sd->bl.m].flag.nozenypenalty) {
			...
			if (zeny_penalty != 0)
				pc->payzeny(sd, zeny_penalty, "Penalty", NULL);
		}
	}
	...
	status->bor_finalizar_effects(sd);
...
int64 pc_readparam(const struct map_session_data *sd, int type)
{
	...
	switch(type) {
		...
		case SP_NOTUNEQUIPELMO:  val = sd->battle_status.unequip_elmo; break;
		case SP_NOTUNEQUIPASA:   val = sd->battle_status.unequip_asa; break;
		case SP_NOTUNEQUIPAURA:  val = sd->battle_status.unequip_aura; break;
...
int pc_setparam(struct map_session_data *sd,int type,int64 val)
{
	...
	case SP_ZENY:
		...
		logs->zeny(sd,"Script", sd, -(sd->status.zeny - cap_value((int32)val, 0, MAX_ZENY)));
...
{
	if( item && (item->expire_time || (item->bound && !pc_can_give_bound_items(sd)) || item->equipSwitch) )
		return 0;
	...
	switch( regname[0] ) {
		...
		case '#':
			if( !strcmp(regname,"#CASHPOINTS") && sd->cashPoints != val ) {
				...
				val = cap_value(val, 0, MAX_ZENY);
				sd->kafraPoints = val;
			} else if( !strcmp(regname,"#UNIVERSEPOINTS") && sd->universePoints != val ) {
				val = cap_value(val, 0, MAX_ZENY);
				sd->universePoints = val;
			}
			break;
	}
...
void pc_equipitem_pos(struct map_session_data *sd, struct item_data *id, int n, int pos)
{
	...
	if (!map_no_view(sd->bl.m,EQP_HEAD_LOW) && pos & EQP_HEAD_LOW && pc->checkequip(sd,EQP_COSTUME_HEAD_LOW,false) == -1) {
	...
	if (!map_no_view(sd->bl.m,EQP_HEAD_TOP) && pos & EQP_HEAD_TOP && pc->checkequip(sd,EQP_COSTUME_HEAD_TOP,false) == -1) {
		if (id)
	...
	if (!map_no_view(sd->bl.m,EQP_HEAD_MID) && pos & EQP_HEAD_MID && pc->checkequip(sd,EQP_COSTUME_HEAD_MID,false) == -1) {
	...
	if (!map_no_view(sd->bl.m,EQP_SHOES) && pos & EQP_SHOES)
		clif->changelook(&sd->bl,LOOK_SHOES,0);
	if (!map_no_view(sd->bl.m,EQP_GARMENT) && pos&EQP_GARMENT && pc->checkequip(sd,EQP_COSTUME_GARMENT,false) == -1) {
...
int pc_equipitem(struct map_session_data *sd,int n,int req_pos, bool equipswitch)
{
	...
	if( !equipswitch && id->flag.bindonequip && !sd->status.inventory[n].bound ) {
...
*/
	if (equipswitch) {
		...
		clif->equipswitch_add(sd, n, pos, false);
		return 1;
	}
	else {
		for (i = 0; i<EQI_MAX; i++) {
...
void pc_unequipitem_pos(struct map_session_data *sd, int n, int pos)
{
	...
	if (pos & EQP_HEAD_LOW && pc->checkequip(sd,EQP_COSTUME_HEAD_LOW,false) == -1) {
		sd->status.head_bottom = 0;
		clif->changelook(&sd->bl,LOOK_HEAD_BOTTOM,sd->status.head_bottom);
	}
	if (pos & EQP_HEAD_TOP && pc->checkequip(sd,EQP_COSTUME_HEAD_TOP,false) == -1) {
		sd->status.head_top = 0;
		clif->changelook(&sd->bl,LOOK_HEAD_TOP,sd->status.head_top);
	}
	if (pos & EQP_HEAD_MID && pc->checkequip(sd,EQP_COSTUME_HEAD_MID,false) == -1) {
		sd->status.head_mid = 0;
		clif->changelook(&sd->bl,LOOK_HEAD_MID,sd->status.head_mid);
	}

	if (pos & EQP_COSTUME_HEAD_TOP) {
		sd->status.head_top = ( pc->checkequip(sd,EQP_HEAD_TOP,false) >= 0 ) ? sd->inventory_data[pc->checkequip(sd,EQP_HEAD_TOP,false)]->look : 0;
		clif->changelook(&sd->bl,LOOK_HEAD_TOP,sd->status.head_top);
	}

	if (pos & EQP_COSTUME_HEAD_MID) {
		sd->status.head_mid = ( pc->checkequip(sd,EQP_HEAD_MID,false) >= 0 ) ? sd->inventory_data[pc->checkequip(sd,EQP_HEAD_MID,false)]->look : 0;
		clif->changelook(&sd->bl,LOOK_HEAD_MID,sd->status.head_mid);
	}

	if (pos & EQP_COSTUME_HEAD_LOW) {
		sd->status.head_bottom = ( pc->checkequip(sd,EQP_HEAD_LOW,false) >= 0 ) ? sd->inventory_data[pc->checkequip(sd,EQP_HEAD_LOW,false)]->look : 0;
		clif->changelook(&sd->bl,LOOK_HEAD_BOTTOM,sd->status.head_bottom);
	}
	...
	if (pos & EQP_GARMENT && pc->checkequip(sd,EQP_COSTUME_GARMENT,false) == -1) {
		sd->status.robe = 0;
		clif->changelook(&sd->bl, LOOK_ROBE, 0);
	}

	if (pos & EQP_COSTUME_GARMENT) {
		sd->status.robe = ( pc->checkequip(sd,EQP_GARMENT,false) >= 0 ) ? sd->inventory_data[pc->checkequip(sd,EQP_GARMENT,false)]->look : 0;
		clif->changelook(&sd->bl,LOOK_ROBE,sd->status.robe);
	}
}
...
int pc_unequipitem(struct map_session_data *sd,int n,int flag)
{
	...
	//pedrodks
	//configuração para não desequipar item de KG
	if (!(flag & PCUNEQUIPITEM_FORCE)) {
		if (sd->battle_status.unequip_elmo >= 6000 || sd->battle_status.unequip_asa >= 6000 || sd->battle_status.unequip_aura >= 6000) {
			if ((sd->battle_status.unequip_elmo == sd->status.inventory[n].id ||
				sd->battle_status.unequip_asa == sd->status.inventory[n].id ||
				sd->battle_status.unequip_aura == sd->status.inventory[n].id)) {
				clif->unequipitemack(sd, 0, 0, UIA_FAIL);
				clif->messagecolor_self(sd->fd, COLOR_CYAN, msg_fd(sd->fd, 3103));//Não é possível desequipar item de habilidade.
				return 0;
			}
		}
	}
...
int pc_equipswitch(struct map_session_data* sd, int index) {
	clif->message(sd->fd, "Recurso Desativado!");
	return -1;
}

void pc_equipswitch_remove(struct map_session_data* sd, int index) {
	struct item* item = &sd->status.inventory[index];
	...
	clif->equipswitch_remove(sd, index, item->equipSwitch, false);
...
int pc_checkitem(struct map_session_data *sd)
{
	...
	for (i = 0; i < sd->status.inventorySize; i++) {
		...
		if (sd->status.inventory[i].equipSwitch & ~pc->equippoint(sd, i)) {
...
int check_time_vip(int tid, int64 tick, int id, intptr_t data)
{
	...
	if(pc_readaccountreg(sd,script->add_variable("#official_time_vip")) < (int)time(NULL)){
		clif->messagecolor_self(sd->fd, COLOR_RED, "Seu tempo de VIP INTEGRAL expirou.");
		save_vip(sd,0);
	}
	if(pc_readaccountreg(sd,script->add_variable("#official_time_vip")) < (int)time(NULL) && pc_readaccountreg(sd,script->add_variable("#official_time_vip_2")) > (int)time(NULL)){
		clif->messagecolor_self(sd->fd, COLOR_RED, "Seu tempo de VIP INTEGRAL expirou. Retornando ao VIP BASICO.");
		save_vip(sd,1);
	}
	
	sd->vip_timer = timer->add(timer->gettick()+DELAY_IN(1),check_time_vip,sd->bl.id,0);
	return 0;
}
 ...
 * Verificação para remoção do vip. [Shiraz / brAthena]
 *-----------------------------------------------------*/
int check_time_vip2(int tid, int64 tick, int id, intptr_t data)
{
	struct map_session_data *sd = (struct map_session_data *)map->id2sd(id);
	...
	if(pc_readaccountreg(sd,script->add_variable("#official_time_vip_2")) < (int)time(NULL)){
		clif->messagecolor_self(sd->fd, COLOR_RED, "Seu tempo de VIP BASICO expirou.");
		save_vip(sd,0);
	}
	
	sd->vip_timer2 = timer->add(timer->gettick()+DELAY_IN2(1),check_time_vip2,sd->bl.id,0);
	return 0;
}
/*======================================================
...
{
	int now = pc_readaccountreg(sd,script->add_variable("#official_time_vip"));
	int val = 0;
	...
	if(now > (int)time(NULL))
		pc_setaccountreg(sd, script->add_variable("#official_time_vip"), now+val);
	else
		pc_setaccountreg(sd, script->add_variable("#official_time_vip"), (int)time(NULL)+val);
...
/*======================================================
 * Adiciona tempo vip. [Haseo]
 *-----------------------------------------------------*/
int add_time_vip2(struct map_session_data *sd, int type[4])
{
	int now = pc_readaccountreg(sd,script->add_variable("#official_time_vip_2"));
	int val = 0;
	...
	if(now > (int)time(NULL))
		pc_setaccountreg(sd, script->add_variable("#official_time_vip_2"), now+val);
	else
		pc_setaccountreg(sd, script->add_variable("#official_time_vip_2"), (int)time(NULL)+val);
...
void show_time_vip(struct map_session_data *sd)
{
	...
	if(sd->group_id==2){
		val = (unsigned int)(pc_readaccountreg(sd,script->add_variable("#official_time_vip")) - (int)time(NULL));
	}else if(sd->group_id==1){
		val = (unsigned int)(pc_readaccountreg(sd,script->add_variable("#official_time_vip_2")) - (int)time(NULL));
	}
	...
	if(sd->group_id==2){
		if(time_s[0] >= 0 && time_s[1] >= 0 && time_s[2] >= 0 && time_s[3] >= 0) {
			snprintf(buf, sizeof(buf), "Sistema VIP INTEGRAL: Restam %d dia(s), %d hora(s), %d minuto(s) e %d segundo(s)", time_s[0], time_s[1], time_s[2], time_s[3]);
			clif->messagecolor_self(sd->fd,COLOR_RED, buf);
		}
		return;
	}else if (sd->group_id==1){
		if(time_s[0] >= 0 && time_s[1] >= 0 && time_s[2] >= 0 && time_s[3] >= 0) {
			snprintf(buf, sizeof(buf), "Sistema VIP BASICO: Restam %d dia(s), %d hora(s), %d minuto(s) e %d segundo(s)", time_s[0], time_s[1], time_s[2], time_s[3]);
...
int pc_update_last_action(struct map_session_data *sd, int type)
{
	...
	switch( type ) {
		...
		case 3:
			...
			{
				int64 mtick = max(sd->keyboard_action_tick,sd->mouse_action_tick);
			...
			break;
		case 4: // Keyboard Possible Actions (Skill Single, Potion)
			if( battle_config.action_keyboard_limit_item )
			{
				if( DIFF_TICK(tick, sd->keyboard_action_tick_item) < battle_config.action_keyboard_limit_item )
					return 0;
				sd->keyboard_action_tick_item = tick;
			}
			break;	
		default:
			break;
...
int pc_readdb(void) {
	...
	while (SQL_SUCCESS == SQL->NextRow(map->brAmysql_handle)) {
		...
		if (maxlv > MAX_LEVEL) {
			ShowWarning("pc_readdb: Nivel maximo %d para classe %d excedeu o limite do servidor (%d).\n ", maxlv, job_id, MAX_LEVEL);
			maxlv = MAX_LEVEL;
		}
		count++;
		job = jobs[0] = pc->class2idx(job_id);
		//We send one less and then one more because the last entry in the exp array should hold 0.
		pc->max_level[job][type] = pc_split_atoui64(split[2], pc->exp_table[job][type],',',maxlv-1)+1;
		...
		if (pc->max_level[job][type] < maxlv) {
			ShowWarning("pc_readdb: Nivel maximo %d para classe %d, mas a tabela de exp dessa classe vai ate %d.\n", maxlv, job_id, pc->max_level[job][type]);
...
void pc_autotrade_load(void)
{
	...
	if (SQL_ERROR == SQL->Query(map->mysql_handle, "SELECT `account_id`,`char_id`,`sex`,`title`,`currency_id` FROM `%s`",map->autotrade_merchants_db))
	...
	while (SQL_SUCCESS == SQL->NextRow(map->mysql_handle)) {
		...
		SQL->GetData(map->mysql_handle, 4, &data, NULL); vloot = atoi(data);

		CREATE(sd, struct map_session_data, 1);

		sd->vend_loot = ( battle_config.extended_vending > 0 ? vloot : 0) ;
...
void pc_autotrade_update(struct map_session_data *sd, enum e_pc_autotrade_update_action action) {
	...
	switch( action ) {
		...
		case PAUC_START: {
			...
			if (SQL_ERROR == SQL->Query(map->mysql_handle, "INSERT INTO `%s` (`account_id`,`char_id`,`sex`,`title`,`currency_id`) VALUES ('%d','%d','%d','%s','%d')",
...
void pc_defaults(void) {
	...
	pc->check_time_vip2 = check_time_vip2;
	pc->add_time_vip = add_time_vip;
	pc->add_time_vip2 = add_time_vip2;
	...
	pc->pc_rankeada = pc_rankeada;
	pc->weaponexp = pc_weaponexp;
	pc->weaponexpup = pc_weaponexpup;
}
