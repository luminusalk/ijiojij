...
void map_flags_init(void) {
	...
	for( i = 0; i < map->count; i++ ) {
		...
		map->list[i].adjust.bexp      = 100; // per map base exp multiplicator
		map->list[i].adjust.jexp      = 100; // per map job exp multiplicator
		if( map->list[i].drop_list != NULL )
			aFree(map->list[i].drop_list);
		map->list[i].drop_list = NULL;
		map->list[i].drop_list_count = 0;

		memset(map->list[i].skill_damage, 0, sizeof(map->list[i].skill_damage));
		memset(&map->list[i].adjust.damage, 0, sizeof(map->list[i].adjust.damage));
		...
		*ptr = '\0';
		...
		if(strcmpi(w1,"timestamp_format")==0)
			...
			if (map->autosave_interval < 1) //Revert to default saving.
				map->autosave_interval = DEFAULT_AUTOSAVE_INTERVAL_MAP;
			else
...
#else
	map->reloadnpc_sub("npc/borutodb/scripts_main.conf");
#endif
...
int inter_config_read(char *cfgName) {
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		else if (strcmpi(w1, "mapsdb") == 0)
			safestrncpy(map->mapsdb, w2, sizeof(map->mapsdb));
...
char *get_database_name(int database_id)
{
	...
	switch(database_id) {
		...
		case 56: db_name = "skill_damage_db"; break;
		case 57: db_name = "lotus_db"; break;
		case 58: db_name = "cartas_db"; break;
		case 59: db_name = "account_cartas"; break;
	}
...
bool map_zone_mf_cache(int m, char *flag, char *params) {
	...
	if (!strcmpi(flag, "nosave")) {
		...
		if( !state ) {
			if( map->list[m].adjust.jexp != 100 ) {
				sprintf(rflag,"jexp\t%d",map->list[m].adjust.jexp);
				map_zone_mf_cache_add(m,rflag);
			}
		} if( sscanf(params, "%d", &state) == 1 ) {
			if( state != map->list[m].adjust.jexp ) {
		...
		if( !state ) {
			if( map->list[m].adjust.bexp != 100 ) {
				sprintf(rflag,"bexp\t%d",map->list[m].adjust.jexp);
				map_zone_mf_cache_add(m,rflag);
			}
		} if( sscanf(params, "%d", &state) == 1 ) {
			if( state != map->list[m].adjust.bexp ) {
	...
	} else if (!strcmpi(flag, "rodexon")) {
		if (state && map->list[m].flag.rodexon)
			; /* nothing to do */
		else {
			if (state)
				map_zone_mf_cache_add(m, "rodexon\toff");
			else if (map->list[m].flag.rodexon)
				map_zone_mf_cache_add(m, "rodexon");
		}
	}
	//pedrodks
	//mapflag sistema ranked
	 else if (!strcmpi(flag, "arena_forfun")) {
		if (state && map->list[m].flag.arena_forfun)
			; /* nothing to do */
		else {
			if (state)
				map_zone_mf_cache_add(m, "arena_forfun\toff");
			else if (map->list[m].flag.arena_forfun)
				map_zone_mf_cache_add(m, "arena_forfun");
		}
	}
	 else if (!strcmpi(flag, "arena_extreme")) {
		if (state && map->list[m].flag.arena_extreme)
			; /* nothing to do */
		else {
			if (state)
				map_zone_mf_cache_add(m, "arena_extreme\toff");
			else if (map->list[m].flag.arena_extreme)
				map_zone_mf_cache_add(m, "arena_extreme");
		}
	}
	 else if (!strcmpi(flag, "arena_ex_cdesab")) {
		if (state && map->list[m].flag.arena_ex_cdesab)
			; /* nothing to do */
		else {
			if (state)
				map_zone_mf_cache_add(m, "arena_ex_cdesab\toff");
			else if (map->list[m].flag.arena_ex_cdesab)
				map_zone_mf_cache_add(m, "arena_ex_cdesab");
		}
	}
	 else if (!strcmpi(flag, "arena_gvg")) {
		if (state && map->list[m].flag.arena_gvg)
			; /* nothing to do */
		else {
			if (state)
				map_zone_mf_cache_add(m, "arena_gvg\toff");
			else if (map->list[m].flag.arena_gvg)
				map_zone_mf_cache_add(m, "arena_gvg");
		}
	}
	 else if (!strcmpi(flag, "noranked")) {
		if (state && map->list[m].flag.noranked)
			; /* nothing to do */
		else {
			if (state)
				map_zone_mf_cache_add(m, "noranked\toff");
			else if (map->list[m].flag.noranked)
				map_zone_mf_cache_add(m, "noranked");
		}
	}
...
int map_zone_str2itemid(const char *name) {
...
#else
	const char *config_filename = "db/borutodb/map_zone_db.conf"; // FIXME hardcoded name
#endif
	...
	if (zones != NULL) {
		...
		for (i = 0; i < zone_count; ++i) {
			...
			if (!libconfig->setting_lookup_string(zone_e, "name", &zonename)) {
				ShowError("map_zone_db: faltando nome da zona, pulando... (%s:%u)\n",
...
static CMDLINEARG(borconfig)
{
	aFree(map->SKILL_CONF_NAME);
	map->SKILL_CONF_NAME = aStrdup(params);
	return true;
}
/**
...
static CMDLINEARG(generatetranslations) {
	...
	if (!(script->lang_export_fp = fopen(script->lang_export_file, "wb"))) {
		...
		fprintf(script->lang_export_fp,
			"# Este arquivo é parte do BrAthena.\n"
			"# http://herc.ws - http://github.com/HerculesWS/Hercules\n"
			"#\n"
			"# Copyright (C) 2013-%d Pedrodks, Brathena old e Hercules Developer team\n"
			"#\n"
			"# Brathena é um software gratuito: Você pode redistribuir e editar como quiser\n"
			"# sob os termos da Licença Pública Geral GNU, publicada pelo\n"
			"# Free Software Foundation, versão 3 de licença, ou\n"
...
void cmdline_args_init_local(void)
{
	...
	CMDLINEARG_DEF2(bor-config, borconfig, "Configuracao alternativa para bor.", CMDLINE_OPT_NORMAL | CMDLINE_OPT_PARAM);
...
int do_init(int argc, char *argv[])
{
	...
	if (!minimal) {
		...
		battle->skill_config_read(map->SKILL_CONF_NAME);
...
void map_defaults(void) {
	...
	map->SKILL_CONF_NAME = "conf/battle/BOR.conf";
